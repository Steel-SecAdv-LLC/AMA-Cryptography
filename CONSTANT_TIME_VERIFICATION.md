...
- **Where a deterministic gate covers a lane, that gate is the blocking authority and the wall-clock lane is informational.** `Kyber-1024 decaps` read |t| = 11.81 in 3/3 rounds at **+5.630 ns** — above the floor, in mispredicted-branch range, and on the FIPS 203 §6.3 implicit-rejection path, where a timing difference is the plaintext-checking oracle the Fujisaki–Okamoto transform exists to deny. It was treated as real until measurement said otherwise. Over 60 decapsulations per class the `kyber-decaps` target reports retired instructions 323,766,461, data memory accesses 168,506,025 and L1 data cache misses 2,651 — all three identical between the valid and the rejected ciphertext. Instructions rule out a branch; the cache figures rule out a secret-dependent access, which instruction counts alone cannot see. At ~5.4M instructions per decapsulation, 5.630 ns is one part in 90,000. The wall-clock lane is therefore INFO and `kyber-decaps` blocks — a strictly more sensitive instrument for the property that matters, resolving a single instruction where the lane cannot resolve 2 ns. This is the pairing `secp256k1 ECDSA sign` already uses with `ecdsa`.
