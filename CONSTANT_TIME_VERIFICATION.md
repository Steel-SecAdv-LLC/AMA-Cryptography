...
- **Three NEON slots are not measurable at all, because the dispatch names do not exist.** `AMA_DISPATCH_ONLY_SLOTS` in `src/c/dispatch/ama_dispatch.c` carries `aes-gcm-neon`, `chacha20-neon` and `sha3-neon` — and those three are measured on AArch64. It carries no `kyber-ntt-neon`, `dilithium-ntt-neon` or `argon2-g-neon`, so the NEON ML-KEM NTT, ML-DSA NTT and Argon2-G kernels — all of which ship in every AArch64 build and every arm64 wheel — cannot be pinned by the sweep even in principle. This one is not a hardware limitation: the hosted `ubuntu-24.04-arm` runners execute NEON natively and already run the three slots that do exist. It is a missing slot, and it is the gap on this list most cheaply closed.
