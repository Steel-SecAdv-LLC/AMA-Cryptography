...
- **A deterministic gate and a wall-clock lane cover different classes of difference, so one does not replace the other.** `Kyber-1024 decaps` read |t| = 11.81 in 3/3 rounds at **+5.630 ns** — above the floor, in mispredicted-branch range, and on the FIPS 203 §6.3 implicit-rejection path, where a timing difference is the plaintext-checking oracle the Fujisaki–Okamoto transform exists to deny. It was treated as real until measurement said otherwise. Over 60 decapsulations per class the `kyber-decaps` target reports, on the Release (-O3) library the wheel ships and with the simulated cache geometry pinned, retired instructions, data references, D1 misses and LLd misses all four identical between the valid and the rejected ciphertext, under gcc 13 and clang 18 alike (the absolute figures for the measuring commit are in `CHANGELOG.md`; the zero deltas are the invariant). Instructions rule out a branch or skipped work; the data-reference and miss figures rule out a secret-dependent access, which instruction counts alone cannot see. (An earlier revision of this bullet argued from the excursion being "one part in 90,000" of a decapsulation; that ratio was computed from an unoptimized build, and the argument does not hold at any denominator, because a mispredicted branch costs a fixed 5–20 ns however long the surrounding operation is. The deterministic identity is the evidence.) The wall-clock lane was then made INFO on the argument that `kyber-decaps` is "a strictly more sensitive instrument for the property that matters, resolving a single instruction where the lane cannot resolve 2 ns". **That argument is withdrawn, and the lane is strict again (2026-09-24).** Callgrind counts retired instructions, data references and simulated misses; a difference that lives only in operand-dependent latency — the same instructions at the same addresses, one of them slower on the reject path — moves none of them at any size. Measured by planting exactly that on the FO verdict in `kyber_decapsulate_internal`: a branch-free mask selecting a subnormal on the reject path and 1.0 on the accept path as the operand of a floating-point multiply (a microcode assist on x86-64). With one such multiply and with 64, `kyber-decaps` passed with every count identical across all eight classes; the wall-clock lane, at 100,000 measurements, read the 64-multiply leak over the threshold in 3 of 3 rounds with one sign in each of two runs (|t| 134–169) — a failure as a strict lane, and a green run under the INFO flag it then carried. The unmutated lane, strict, passed five of five runs in one round each on the same host (|t| ≤ 2.24); the excursion that first made it INFO is accounted for below. So `kyber-decaps` blocks any change in instruction sequence or memory access, down to one instruction, and the wall-clock lane blocks any difference of 2 ns or more, including a latency-only one; below 2 ns a latency-only difference is measured by neither (see above). `secp256k1 ECDSA sign` remains INFO with `ecdsa` blocking, and that pairing has the same blind spot. **Where the excursion came from is now measured rather than inferred:** the lane selected its ciphertext with a class-correlated ternary sitting between the class draw and the opening timer, and on a null experiment — byte-identical ciphertexts in both classes, so the true effect is exactly zero, 200,000 measurements x 5 runs — that construction reads over threshold in 3 of 5 runs at worst |t| = 6.99 with every excursion the same sign, while the masked-merge staging the lane now uses reads 0 of 5. An earlier revision of this bullet attributed the residual to data-operand-dependent execution (Intel DOITM / ARM PSTATE.DIT); that inference is withdrawn as unnecessary. The deterministic identity is unchanged and is what settles the primitive.
...
`tests/c/test_dudect.c` registers **seven** info-only lanes, and here is every
...
`Kyber-1024 decaps` was the eighth until 2026-09-24, when it was made strict
again: `--target kyber-decaps` sees instruction-sequence and memory-access
differences, not one that lives only in operand-dependent latency, so it
could not stand in for the timing lane (see the lane's comment).

An earlier version of this paragraph named six of the then eight and asserted that
