 ...
 */
static ama_error_t slh_thash_wots(uint8_t *out, const uint8_t *in,
                                    const uint8_t *pub_seed, const slh_addr_t addr,
                                    const slh_params_t *p) {
    ...
    /* FIX: Return AMA_ERROR_MEMORY on malloc failure instead of silent
     * zeroing, which would produce an invalid public key / signature. */
...
static void slh_h_msg(uint8_t *out, size_t out_len,
    ...
    /* FIX: Check for integer overflow before malloc.
     * If input_len + 4 wraps around, the malloc would succeed with a
     * tiny buffer and the subsequent memcpy would write out-of-bounds. */
    if (input_len > SIZE_MAX - 4) { memset(out, 0, out_len); return; }
...
static void slh_wots_chain(uint8_t *out, const uint8_t *in,
                             ...
                             const slh_params_t *p) {
    /* FIX: Constant-time WOTS+ chain computation.
     * The original loop count depended on 'steps' (derived from message
     * digest nibble), leaking timing information. Now we always iterate
     * W-1 steps and use constant-time conditional output selection. */
    unsigned int i;
    uint8_t tmp[SLH_N_MAX];
    memcpy(out, in, p->n);
    for (i = 0; i < SLH_WOTS_W - 1; i++) {
        slh_set_hash(addr, i);
        memcpy(tmp, out, p->n);
        slh_thash_f(tmp, tmp, pub_seed, addr, p);
        /* Constant-time conditional copy: overwrite out only if
         * start <= i < start + steps.  Uses arithmetic masks to
         * avoid branches that depend on the message digest. */
        unsigned int in_range = ((i - start) < steps) & (i >= start);
        /* Convert boolean to byte mask: 0x00 or 0xFF */
        uint8_t mask = (uint8_t)(-(int8_t)in_range);
        for (unsigned int b = 0; b < p->n; b++) {
            out[b] = (tmp[b] & mask) | (out[b] & ~mask);
        }
    }
}
