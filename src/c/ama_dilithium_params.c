...
static void ml_poly_uniform(ml_poly *a, const uint8_t rho[MLDSA_SEEDBYTES],
                              uint16_t nonce) {
    /* FIX: Incremental SHAKE128 squeezing.
     * The previous fixed 768-byte buffer had ~22% exhaustion rate for Q~2^23.
     * Now we squeeze in 168-byte (SHAKE128 rate) blocks until all 256
     * coefficients are sampled via rejection. */
    uint8_t buf[MLDSA_SEEDBYTES + 2];
    uint8_t stream[168 * 5]; /* Initial squeeze: 5 blocks = 840 bytes */
    unsigned int ctr, pos;
    size_t stream_len;
    ...
    stream_len = sizeof(stream);
    ama_shake128(buf, MLDSA_SEEDBYTES + 2, stream, stream_len);

    ctr = 0;
    pos = 0;
    while (ctr < MLDSA_N) {
        /* If we've exhausted the current stream, squeeze more */
        if (pos + 3 > stream_len) {
            /* Re-squeeze from the same seed with increased output length.
             * This is a simplified approach — a full incremental SHAKE128
             * state would be more efficient but requires a streaming API. */
            stream_len = sizeof(stream);
            ama_shake128(buf, MLDSA_SEEDBYTES + 2, stream, stream_len);
            pos = 0;
            /* Skip already-consumed portion by re-running rejection */
            unsigned int skip_ctr = 0;
            unsigned int skip_pos = 0;
            while (skip_ctr < ctr && skip_pos + 3 <= stream_len) {
                uint32_t t;
                t = stream[skip_pos++];
                t |= (uint32_t)stream[skip_pos++] << 8;
                t |= (uint32_t)stream[skip_pos++] << 16;
                t &= 0x7FFFFF;
        ...
        if (pos + 3 > stream_len) break; /* Safety: shouldn't happen */
...
static int32_t ml_poly_chknorm(const ml_poly *a, int32_t B) {
    /* FIX: Constant-time norm check.
     * The original early-return leaked timing information about which
     * coefficient (if any) exceeded the bound. Now we process ALL
     * coefficients and accumulate the result with constant-time OR. */
    unsigned int i;
    int32_t fail = 0;
    for (i = 0; i < MLDSA_N; ++i) {
        int32_t t = a->coeffs[i] >> 31;
        t = a->coeffs[i] - (t & 2 * a->coeffs[i]);
        /* Constant-time: fail |= (t >= B) without branching */
        fail |= (B - 1 - t) >> 31;
    }
    return (fail != 0) ? 1 : 0;
}
...
static ama_error_t ml_dsa_sign(uint8_t *sig, size_t *sig_len,
    ...
    for (n = 0; n < 1000; ++n) {
        ...
        /* Pack signature: c_tilde || z || h */
        {
            ...
            /* Pack z */
            /* FIX: Zero the z-packing buffer before OR-ing.
             * Without this memset, stale bits from previous rejection
             * loop iterations could corrupt the packed signature. */
            memset(sig_ptr, 0, p->l * p->polyz_packed);
            for (i = 0; i < p->l; ++i) {
                for (j = 0; j < MLDSA_N; ++j) {
    ...
    /* Unpack z (simplified: use rejection bounds check) */
    {
        ...
        for (i = 0; i < p->l; ++i) {
            for (j = 0; j < MLDSA_N; ++j) {
                ...
                if (p->gamma1 == (1 << 17)) {
                    /* FIX: Correct 18-bit unpacking for gamma1=2^17.
                     * The original had wrong bit alignment that didn't
                     * match pqcrystals polyz_unpack for 18-bit values. */
                    unsigned int byte_off = (j * 18) / 8;
                    unsigned int bit_off = (j * 18) % 8;
                    int32_t val = 0;
                    if (byte_off + 2 < p->polyz_packed) {
                        uint32_t raw = (uint32_t)z_ptr[byte_off]
                                     | ((uint32_t)z_ptr[byte_off + 1] << 8)
                                     | ((uint32_t)z_ptr[byte_off + 2] << 16);
                        val = (int32_t)((raw >> bit_off) & 0x3FFFF);
                    }
