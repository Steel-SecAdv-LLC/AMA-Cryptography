 ...
 *              default on every host and the only path on AArch64 and on
 *              MSVC.
 *   fe64-mulx  radix 2^64, four limbs, the MULX+ADX multiply and square
 *              kernel.  Instantiated in src/c/x86/ama_ed25519_fe64_mulx.c and
 *              compiled per-file with -mbmi2 -madx on x86-64 GCC/Clang.  It
 *              runs only after ama_ed25519_set_mulx_override(1), and then
 *              only when ama_cpuid_has_x25519_mulx() reports BMI2 and ADX —
 *              the same CPU gate X25519 uses.  With the override at its
 *              default (-1) every call runs fe51, BMI2+ADX host or not: the
 *              default is set by measurement, not by ISA (BACKEND DISPATCH
 *              below; tests/c/test_ed25519_fe51_mulx_equiv.c pins it).
 *
