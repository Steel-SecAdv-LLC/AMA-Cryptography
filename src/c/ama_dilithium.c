...
static void dil_poly_challenge(dil_poly *c, const uint8_t seed[DIL_CTILDEBYTES]) {
    ...
    memset(c->coeffs, 0, sizeof(c->coeffs));  /* PUBLIC-DATA: Dilithium challenge polynomial coefficients init (c is derived from public msg + public commitment per FIPS 204). */
...
static void dil_polyveck_use_hint(dil_polyveck *w, const dil_polyveck *v,
    ...
    memset(hint_flags, 0, sizeof(hint_flags));  /* PUBLIC-DATA: Dilithium hint-flag bitfield init (unpacking from public signature hint bytes). */
    k_idx = 0;
    for (i = 0; i < DIL_K; ++i) {
...
AMA_API ama_error_t ama_dilithium_sign(uint8_t *signature, size_t *signature_len,
    ...
    memset(hint, 0, sizeof(hint));  /* PUBLIC-DATA: Dilithium signature hint-bytes init (FIPS 204 sig.h component is public; sent on the wire). */
    ...
    /* PUBLIC-DATA: FIPS 204 deterministic-signing rnd component = 0^256 (intentional zero bytes per the spec; not a scrub). */
    ...
    while (reject) {
        ...
        memset(hint, 0, sizeof(hint));  /* PUBLIC-DATA: Dilithium signature hint-bytes reset per rejection-sampling iteration (sig component is public). */
