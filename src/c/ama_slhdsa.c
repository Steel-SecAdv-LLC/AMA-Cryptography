...
static void sha2_F(const slhdsa_params_t *p, uint8_t *out,
    ...
    memset(padding, 0, sizeof(padding));  /* PUBLIC-DATA: FIPS 205 toByte(0,128-n) zero-pad bytes (hash-input domain separator). */
...
static void sha2_HT(const slhdsa_params_t *p, uint8_t *out,
    ...
    /* PUBLIC-DATA: SHA2_HT input buffer init — composes PK.seed || toByte(0,128-n) || ADRSc || M; padding region intentionally kept as zeros (FIPS 205 §11.2). */
    memset(buf, 0, total);
...
static void slh_ht_sign(const slhdsa_params_t *p, uint8_t *sig,
    ...
    uint8_t root[32];
    size_t layer;
    memset(addr, 0, sizeof(addr));  /* PUBLIC-DATA: SLH-DSA ADRS struct init (32-byte address word; tree path is public) */
...
static ama_error_t slh_sign_internal(const slhdsa_params_t *p,
    ...
    memset(fors_addr, 0, sizeof(fors_addr));  /* PUBLIC-DATA: SLH-DSA FORS ADRS struct init (32-byte address word; tree path is public) */
