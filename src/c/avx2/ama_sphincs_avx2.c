 ...
 * ============================================================================ */
/* Forward declaration with external linkage — mirrors the test
 * harness's `extern` declaration in tests/c/test_sphincs_simd_equiv.c
 * and silences `-Wmissing-prototypes`. */
void ama_sphincs_wots_chain_avx2(uint8_t *out, const uint8_t *in,
                                 uint32_t start, uint32_t steps,
                                 const uint8_t *pub_seed,
                                 uint32_t addr[8], size_t n);

/* Function visibility: kept extern (not `static`) so the
 * test harness `tests/c/test_sphincs_simd_equiv.c` can pair it
 * directly against the scalar SHA-256 wots-chain reference for a
 * byte-identity equivalence check.  Production code does not call
 * this helper today (the production SPHINCS+ sign / verify pipeline
 * runs the scalar SHA-256 path); exposing the symbol does not
 * change runtime behaviour, but it does let the test surface
 * `wots_chain_avx2` regressions in the same PR that introduces them
 * rather than waiting for a future production wiring. */
