...
static inline __m128i gcm_inc_counter_avx512(__m128i cb) {
    /* Big-endian 32-bit increment of the last 4 bytes using _mm_add_epi32.
     * Byte-swap the counter lane, add 1, byte-swap back.
     * This avoids store-to-memory round-trips and is fully in-register. */
    const __m128i bswap32_mask = _mm_set_epi8(
        12, 13, 14, 15, 8, 9, 10, 11, 4, 5, 6, 7, 0, 1, 2, 3);
    const __m128i one = _mm_set_epi32(1, 0, 0, 0);
    __m128i swapped = _mm_shuffle_epi8(cb, bswap32_mask);
    swapped = _mm_add_epi32(swapped, one);
    return _mm_shuffle_epi8(swapped, bswap32_mask);
}
