...
ama_error_t ama_sha3_256(
    ...
    memset(block, 0, sizeof(block));  // PUBLIC-DATA: block (SHA3 rate-block padding buffer, pre-use init filled by memcpy + 0x06...0x80 padding bytes)
    if (remaining > 0) {
    ...
    const uint8_t *in3, size_t in3_len)
{
    ...
    for (int lane = 0; lane < 4; lane++) {
        ...
        memset(block, 0, sizeof(block));  // PUBLIC-DATA: block (per-lane SHAKE x4 padding buffer, pre-use init filled from ctx->buffers)  // PUBLIC-DATA: block (SHA3 rate-block padding buffer, pre-use init filled by memcpy + 0x06...0x80 padding bytes)
        if (in_lens[lane] > 0) {
