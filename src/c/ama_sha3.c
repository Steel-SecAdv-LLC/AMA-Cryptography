...
ama_error_t ama_sha3_256(
    ...
    memset(state, 0, sizeof(state));  /* PUBLIC-DATA: SHA3 sponge state init (pre-absorb; exit scrub uses ama_secure_memzero) */
    ...
    memset(block, 0, sizeof(block));  /* PUBLIC-DATA: SHA3 padding block init (zero-fill then 0x06/0x80 padding bytes) */
    if (remaining > 0) {
...
ama_error_t ama_sha3_init(ama_sha3_ctx* ctx) {
    ...
    memset(ctx->state, 0, sizeof(ctx->state));  /* PUBLIC-DATA: SHA3 streaming-context sponge state init (pre-absorb) */
    memset(ctx->buffer, 0, sizeof(ctx->buffer));  /* PUBLIC-DATA: SHA3 streaming-context per-chunk buffer init (overwritten by next absorb) */
    ctx->buffer_len = 0;
    ctx->finalized = 0;
    ...
    const uint8_t *in3, size_t in3_len)
{
    ...
    memset(ctx->states, 0, sizeof(ctx->states));  /* PUBLIC-DATA: SHAKE x4 streaming-context per-lane sponge state init (pre-absorb) */
