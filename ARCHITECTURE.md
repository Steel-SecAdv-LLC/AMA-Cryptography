...
| C Unit Tests | Native library validation | All C functions | 61 `test_*.c` registered via ctest in `tests/c/` (+ 1 standalone `bench_*.c` + 2 standalone `x25519_equiv_*.c`) |
...
**Total:** 4,435 Python test functions across 183 test files, plus the
