...
| Keccak-f[1600] (SHA3, SHAKE) | AVX2 4-way (Keccak-f[1600] across 4 SIMD lanes) | Build: default ON. Runtime: `ama_has_avx2()` | ~3-4× over scalar Keccak | `src/c/avx2/ama_sha3_avx2.c` |
...
| ChaCha20-Poly1305 | 8-way AVX2 ChaCha20 block function (512 B keystream per kernel invocation) | Runtime: `ama_has_avx2()`. Opt-out: `AMA_DISPATCH_NO_CHACHA_AVX2=1` | 2.11× at 1 KB, 2.24× at 4 KB, 2.29× at 64 KB; messages < 512 B stay on scalar | `src/c/avx2/ama_chacha20poly1305_avx2.c`, `tests/c/test_chacha20poly1305.c` |
| Argon2id | 4-way BlaMka G AVX2 (`_mm256_mul_epu32` for the multiplication-hardened add; `_mm256_permute4x64_epi64` for the diagonal pass) | Runtime: `ama_has_avx2()`. Opt-out: `AMA_DISPATCH_NO_ARGON2_AVX2=1` | 1.31× at m=64 KiB, 1.34× at m=1 MiB | `src/c/avx2/ama_argon2_avx2.c`, `tests/c/test_argon2id.c` |
...
| ML-DSA-65 / ML-KEM-1024 sampling | 4-way SHAKE128 / SHAKE256 across 4 SIMD lanes; CBD2 noise sampling AVX2-vectorised | Runtime: `ama_has_avx2()` | Throughput-bound by the SHAKE rounds; sign / encaps ~3× faster than the scalar reference on this host | `src/c/avx2/ama_*_avx2.c` (PR #260) |
