...
- Field arithmetic in radix 2^51 (`fe51`, `src/c/fe51.h`), the default on every host, with a dedicated squaring `fe51_sq()` — 15 64x64-bit limb products against 25 for the general `fe51_mul()`. A radix-2^64 MULX/ADX instantiation (`src/c/x86/ama_ed25519_fe64_mulx.c`) is compiled on x86-64 and produces byte-identical results, but it is reachable only through `ama_ed25519_set_mulx_override()`, a benchmark/test knob: by measurement it ties fe51 on keygen and sign and loses on verify
- No run-time initialisation and no `_Atomic` or lock on the Ed25519 path: the base-point tables are `static const`, generated by `tools/gen_ed25519_tables.py`, and nothing in the file writes file-scope state
- Constant-time fixed-base comb for the secret scalar: a fixed digit count and a masked fold that reads every entry of the table row (the AVX2 kernel `src/c/avx2/ama_ed25519_select_avx2.c` where `ama_has_avx2()` holds, the SSE2 fold on other x86-64 hosts, a portable masked fold elsewhere)
- Validated against RFC 8032 Test Vector 1 (12 test vectors)
- MSVC builds use the same `fe51` path: its 128-bit products come from `_umul128` / `__shiftright128` (x64) or `__umulh` (ARM64) where the compiler has no `unsigned __int128`

**Performance.** The committed benchmark record (`benchmark-report.md`, from
`benchmarks/benchmark-results.json`) carries two Python-API signing rows, and
they are different operations. `ed25519_sign` takes the 64-byte `seed || A`
key on every call; INVARIANT-51 re-derives `A = [a]B` from the seed each
time and refuses a key whose stored `A` disagrees — the stored half is a
layout, not a cache.
`ed25519_sign_expanded` loads the key once through `Ed25519SigningKey`, doing
that derivation at load, so each signature re-checks the expanded form's tag
instead. The ratio between the two rows is the per-signature cost of the
derivation. The figures this section quoted until 5.0.0 (refreshed 2026-04-21)
predate INVARIANT-51 and described a signing path that trusted the stored `A`;
they were removed rather than restated.
