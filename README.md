...
> **Note:** All PQC operations are provided by the native C library (libama_cryptography). No external PQC dependencies (liboqs, pqcrypto) are required. Build with `cmake -B build -DAVA_USE_NATIVE_PQC=ON && cmake --build build`. Set `AVA_REQUIRE_CONSTANT_TIME=true` to enforce constant-time operation at runtime.
