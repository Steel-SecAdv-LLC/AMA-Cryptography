...
| X25519 Scalar-mult (fe64 + MULX+ADX kernel) | 13,988 ops/sec | canonical bench, 2026-04-26 |

**Performance Note:** Ed25519 signing stores the expanded 64-byte key (seed||pk) to avoid redundant SHA-512 expansion on each sign call. X25519 now uses the radix-2^64 (`fe64.h`) field arithmetic on x86-64 GCC/Clang (default) with the radix-2^51 (`fe51.h`) layout retained as a fallback for non-x86-64 64-bit GCC/Clang and the portable radix-2^16 path retained for MSVC and 32-bit targets. On hosts where CPUID reports both BMI2 (`EBX[8]`) and ADX (`EBX[19]`), the dispatcher additionally promotes the inner ladder's multiply / square to the in-house MULX+ADCX/ADOX kernel in `src/c/internal/ama_x25519_fe64_mulx.c` — gated by `ama_cpuid_has_x25519_mulx()` and pinned byte-identical to the pure-C fe64 reference across 4096 random vectors by `tests/c/test_x25519_fe64_mulx_equiv.c`. The wiring is correctness-equivalent across all three field paths (verified by 1024 random vectors in `tests/c/test_x25519_field_equiv.c`); on this build the MULX+ADX kernel improves throughput from the pure-C fe64 baseline of ~11,500 ops/sec to ~13,988 ops/sec (~21 %). The remaining gap to OpenSSL's hand-tuned `crypto/ec/asm/x25519-x86_64.pl` (~25K ops/sec on this microarchitecture class) is the dual-carry-chain ADCX/ADOX interleave that intrinsic-driven C code cannot reliably emit — a future hand-written `.S` kernel slotting behind the same CPUID gate captures the rest of the curve. See [benchmarks/](benchmarks/) for full performance data including all algorithms.
