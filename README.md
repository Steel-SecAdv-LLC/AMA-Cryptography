...
[![PQC](https://img.shields.io/badge/PQC-ML--DSA--65%20%7C%20ML--KEM--1024%20%7C%20SLH--DSA-purple.svg)](CRYPTOGRAPHY.md)
...
| ML-DSA-44 / -65 / -87 | **Full** (native) | Full | Integrated; `ama_ml_dsa_*` / `native_ml_dsa_*` |
| ML-KEM-512 / -768 / -1024 (Kyber lineage) | **Full** (native) | Full | Integrated; `ama_ml_kem_*` / `native_ml_kem_*` |
| SLH-DSA-SHA2-256f | **Full** (native) | Full | Integrated |
| SLH-DSA-SHAKE-128s | **Full** (native) | Full | Integrated |
| X25519 | **Full** | Full | Key exchange |
| ChaCha20-Poly1305 | **Full** | Full | Alternative AEAD |
| Argon2 | **Full** | Full | Password hashing |
| secp256k1 | **Full** | Full | HD key derivation, ECDSA |
...
- `ama_x25519.c`: X25519 Diffie-Hellman key exchange (RFC 7748) with `ama_x25519_scalarmult_batch` API
- `ama_chacha20poly1305.c`: ChaCha20-Poly1305 AEAD (RFC 8439)
- `ama_argon2.c`: Argon2id password hashing (RFC 9106) — byte-identical to spec; `out_len` capped at `AMA_ARGON2ID_MAX_TAG_LEN = 1024`. Legacy verify-only path (`ama_argon2id_legacy` / `ama_argon2id_legacy_verify`) shipped for one-shot migration of hashes generated by AMA ≤ 2.1.5 (see CHANGELOG `[3.0.0] → ### BREAKING`).
- `ama_secp256k1.c`: secp256k1 elliptic curve operations (HD key derivation, ECDSA)
- `ama_nistp.c`: NIST P-256/P-384/P-521 — ECDSA (FIPS 186-5) and ECDH (SP 800-56A)
- `ama_aes_bitsliced.c`: Bitsliced AES S-box (cache-timing hardened, **default**: `AMA_AES_CONSTTIME=ON`)
- `fe51.h`: Radix-2⁵¹ field arithmetic (5×uint64 + `__int128`) — Ed25519 / X25519 fallback for non-x86-64 64-bit targets
- `fe64.h`: Radix-2⁶⁴ field arithmetic (4×uint64 schoolbook + `__uint128_t`) — X25519 default on x86-64 GCC/Clang
- `internal/ama_sha2.h`: Extracted SHA-512 header-only implementation (deduplication for Ed25519 / SPHINCS+ / FROST)
- `internal/ama_sha3_x4.h`: 4-way Keccak-f[1600] interface shared by AVX2 and AVX-512 backends
- `internal/ama_x25519_fe64_mulx.c`: Hand-written GCC/Clang inline-asm 4×4 X25519 fe64 multiply / square / reduce using `mulx` (BMI2) + `adcx` / `adox` (ADX) dual-carry chains; gated by `ama_cpuid_has_x25519_mulx()` (BMI2 ∧ ADX bundle)

**Hand-Written SIMD Implementations (`src/c/avx2/`, `src/c/avx512/`, `src/c/neon/`, `src/c/sve2/`) — measured 2026-05-16: 26 SIMD `.c` files (9 AVX2 + 1 AVX-512 + 8 NEON + 8 SVE2):**
- The Ed25519 AVX2 path was removed in PR #238 because the scalar `fe51` field-arithmetic backend was already faster than the vector trampoline; NEON retains its Ed25519 entry, and SVE2 Ed25519 is intentionally not wired (no batched single-key API consumes a SIMD ladder — see `src/c/sve2/ama_ed25519_sve2.c`).
- `avx2/` (9 files): ML-KEM (vectorized NTT/Barrett, batch CBD2 sampling), ML-DSA (vectorized NTT q=8380417, batch SHAKE rejection), SPHINCS+ (4-way SHA-256), SHA3 (Keccak-f[1600] 4-way AVX2), AES-GCM (pipelined AES-NI + PCLMULQDQ GHASH), **VAES YMM AES-256-GCM** (`ama_aes_gcm_vaes_avx2.c` — clean replacement, wins land at ≥ 4 KB block sizes), ChaCha20-Poly1305 (8-way parallel block function, ≥ 512 B ≈ 2.1–2.3×), Argon2 (4-way BlaMka G over AVX2 YMM, ≈ 1.3×), **X25519 4-way Montgomery ladder + `ama_x25519_scalarmult_batch` API** (`ama_x25519_avx2.c` — opt-in via `AMA_DISPATCH_USE_X25519_AVX2=1`; intentionally not faster than scalar fe64 on MULX/ADX hosts; retained for CI matrix coverage and a future AVX-512 IFMA port)
- `avx512/` (1 file, opt-in via `-DAMA_ENABLE_AVX512=ON`): in-house EVEX-encoded YMM-width 4-way Keccak permutation kernel (`ama_sha3_x4_avx512.c` — `vprolq` for the 64-bit rotate, `vpternlogq` for the theta `0x96` and chi `0xD2` collapses). XCR0 5+6+7-gated so an EVEX YMM op cannot `#UD` on a host whose hypervisor advertises CPUID without the ZMM save area. See `docs/AVX512_KECCAK_ADR.md` ADR for the in-house-vs-vendored rationale.
- `neon/` (8 files): ARM NEON 128-bit vector equivalents using `<arm_neon.h>` intrinsics + ARM Crypto Extensions (Ed25519, ML-KEM, ML-DSA, SPHINCS+, SHA3, AES-GCM, ChaCha20-Poly1305, Argon2)
- `sve2/` (8 files): ARM SVE2 scalable vector implementations. **Wired surface (3 of 8 TUs):** SHA3/Keccak permutation, ML-KEM-1024 NTT trio, ML-DSA-65 NTT trio. The remaining five TUs (`ama_aes_gcm_sve2.c`, `ama_chacha20poly1305_sve2.c`, `ama_argon2_sve2.c`, `ama_sphincs_sve2.c`, `ama_ed25519_sve2.c`) are documentation placeholders — their per-file headers enumerate the concrete reasons each kernel cannot be wired today (dispatch-signature mismatch, algorithmic incorrectness vs. RFC 9106 BlaMka, absent dispatch surface, no production batched caller) and the preconditions a future SVE2 kernel must meet before wiring. SVE2 hosts dispatch those five algorithms to the validated NEON kernels under `src/c/neon/`, which is a strict upgrade over the generic-C fallback.
- `dispatch/ama_dispatch.c`: Runtime CPU feature detection and function-pointer dispatch (AVX-512 > AVX2 > generic on x86 *for the SHA3 slot only*; AVX2 > generic for every other x86 slot; SVE2 > NEON > generic on ARM). The `AMA_IMPL_AVX512` slot is real and promotes the SHA3 dispatch entry to the AVX-512 4-way kernel when `AMA_ENABLE_AVX512=ON` at build time and `ama_cpuid_has_avx512_keccak()` (AVX-512F + AVX-512VL + XCR0 5+6+7) holds at runtime; every other slot keeps the existing AVX2 ceiling until it grows its own ZMM kernel. Best-of-5 SHA-3 auto-tune with a 10% revert threshold replaced the prior single-shot comparison that was a flake source on contended runners. Override env: `AMA_DISPATCH_NO_AUTOTUNE=1`, `AMA_DISPATCH_VERBOSE=1`.

**Cython Acceleration Modules (`src/cython/`) — 7 `.pyx` files:**
...
- `hkdf_binding.pyx`: Direct Cython binding to `ama_hkdf_sha3_256()`
- `ed25519_binding.pyx`: Direct Cython binding to `ama_ed25519_sign()` / `ama_ed25519_verify()`
- `dilithium_binding.pyx`: Direct Cython binding to `ama_dilithium_sign()` / `ama_dilithium_verify()`
...
> **Reading the numbers below.** All ops/sec figures in the tables that follow are from the **canonical bench host** (Linux x86-64 with AVX-512F/VL/BW/DQ/VBMI + VAES + VPCLMULQDQ; Sapphire Rapids / Zen 4 class), measured 2026-04-25 with `python benchmarks/benchmark_runner.py` and `build/bin/benchmark_c_raw --json`. The checked-in `benchmarks/benchmark-results.json` and `benchmarks/baseline.json` carry a separate, lower set of numbers — the **slow-runner CI regression floor** — so even contended GitHub Actions shared runners clear the regression threshold without false-positive failures. The two are not the same number, on purpose. Reproduce the canonical numbers on equivalent silicon; the regression floor is documented in [CHANGELOG.md](CHANGELOG.md#300---2026-04-27) §"Slow-runner regression-floor recalibration".
...
*Twelve-panel performance dashboard: operation throughput (log axis), performance by category, signature latency with ±σ error bars, package scalability, key-generation speed, the 4-layer package time breakdown, measured-vs-regression-floor margins, claimed-vs-measured latency, Ed25519 against peer libraries, ethical-integration overhead, NIST FIPS implementation status, and a run summary. Every panel is regenerated by `tools/generate_dashboards.py` from four measurement artefacts — `benchmark_results.json` (from `benchmarks/benchmark_suite.py`), `benchmarks/regression_results.json` (`benchmark_runner.py --output`), `benchmarks/validation_results.json` (`validation_suite.py`) and `benchmarks/comparative_benchmark_results.json` (`comparative_benchmark.py`). No hand-entered figures: a panel whose artefact is absent renders the command that produces it rather than a substituted number.*
...
| secp256k1 ECDSA Sign (fixed-base comb) | 2,887 ops/sec | **sandbox** bench, 2026-07-29 |
| secp256k1 ECDSA Verify (Shamir's trick) | 4,137 ops/sec | **sandbox** bench, 2026-07-29 |

> The two secp256k1 rows are **sandbox** measurements, not canonical-host ones — the rest of this table is canonical. They are marked rather than omitted because the underlying algorithm changed on 2026-07-29 and the old canonical figure no longer describes the code. Re-measure on canonical silicon before quoting them externally; the raw-C numbers and the full before/after are in [docs/BENCHMARK_HISTORY.md](docs/BENCHMARK_HISTORY.md#2026-07-29-secp256k1-fixed-base-comb--baseline-validity-window-enforcement).

**secp256k1 fixed-base comb (2026-07-29):** public-key derivation and the ECDSA signing nonce both compute `d·G` against the compile-time generator, and both previously went through the generic Montgomery ladder — one addition *and* one doubling per scalar bit, because the ladder must not branch. That is the right shape for a caller-supplied base and pure waste for a constant: on one host `d·G` cost 351.93 µs on secp256k1 against 138.16 µs for the equivalent P-256 operation, even though secp256k1's prime (2^256−2^32−977) reduces *faster* than P-256's — the whole gap was the algorithm. A 4-block comb (16 entries, ~1.9 KB, L1-resident, the same construction `ama_nistp.c` already used for the NIST generators) replaces 256 doublings and 256 additions with 64 of each: raw-C pubkey derivation **354.97 → 83.36 µs (4.26×)** and ECDSA signing **392.94 → 125.54 µs (3.13×)**. Verification is deliberately untouched — it is variable-time by design and already uses Shamir's trick — and `ama_secp256k1_point_mul` keeps the ladder, because precomputing for a caller-supplied base buys nothing and a table built from attacker-supplied input is a surface the library does not need. Constant-time is preserved and measured, not asserted: the scalar is read a bit at a time at fixed indices and the table by masked full-table scan, giving Welch's t-test `|t| = 0.29` fixed-vs-random and `|t| = 1.03` on a Hamming-weight split over 60,000 samples, against dudect's leakage threshold of 4.5.
...
### Competitive Position & Standardized Performance

Two interactive pages, generated from measured artefacts rather than hand-written numbers:

| Page | What it answers |
|------|-----------------|
| [`benchmarks/competitive.html`](benchmarks/competitive.html) | How AMA compares to **libsodium** (PyNaCl) and **OpenSSL** (`cryptography`) on the same host, same process, same parameter sets — plus **cycles/byte** and MB/s, the metrics eBACS and Crypto++ report, and a message-size scalability sweep |
| [`benchmarks/dashboard.html`](benchmarks/dashboard.html) | Throughput by primitive, the secp256k1 comb before/after, and regression-gate headroom against the recalibrated floors |

Both wins and losses are shown. AMA leads on signature operations — Ed25519 verify **3.04×** OpenSSL and **1.43×** libsodium, ML-DSA-65 sign **2.22×** OpenSSL — and trails on ML-KEM encapsulation (**2.81×**) and on bulk AES-GCM (**4.91×** at 64 KiB). The AES gap is a deliberate posture, not a deficiency: this CPU exposes AES-NI and OpenSSL uses it, while AMA defaults to a **constant-time bitsliced** AES (INVARIANT-20) that never indexes a table with key-dependent data. Table-based AES exists only behind the opt-in `AMA_AES_TABLE_INSECURE` flag. Peer libraries are benchmark-only comparison targets and are not linked into the production library — INVARIANT-36 forbids a peer implementation as a *correctness* answer key, and a speed reference is not one.

Reproduce with `python benchmarks/comparative_benchmark.py` and `./build/bin/benchmark_c_raw`.
...
- `AMA_BUILD_FUZZ` - Build coverage-guided libFuzzer harnesses (default: OFF; 13 targets in `fuzz/`)
...
- `AMA_ED25519_VERIFY_SHAMIR` - Use Shamir / Straus joint `[s]B + [h](-A)` in Ed25519 verify (default: ON; set OFF for legacy split scalar-mult, useful for deterministic regression comparisons)
- `AMA_ED25519_VERIFY_WINDOW` - wNAF window width for Ed25519 verify scalar mults; integer in `[2, 6]` (default: `5`)
...
- **Rigorous testing:** 2,159 test functions across 77 Python files plus 37 C files, anchored in [docs/METRICS_REPORT.md](docs/METRICS_REPORT.md); CI includes security scanning, NIST ACVP validation (1,215/1,215 — 815 AFT + 400 SHA-3 MCT), and tiered benchmark-regression checks
