...
| Library total (Python + C + headers) | 118 | **95,885** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,460 |
| Tests (`tests/**/*.py`) | 275 | 125,262 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 783 | **465,256** |

**Library total (the figure that most closely tracks "library size"):
95,885 lines** across 118 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`465,256` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 95,885 | 20.6% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 125,262 | 26.9% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 240,751 | 51.7% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **465,256** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **6,549** |
