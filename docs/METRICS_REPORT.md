...
| **Whole project** (source + docs + config) | 728 | **411,016** |
...
**Whole-project total** (`411,016` lines across Python, C, headers,
...
| Tests | 102,795 | 25.0% | `tests/**/*.py` |
| Top-level Python | 1,403 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 211,840 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **411,016** | **100%** | sum of the scopes above |
