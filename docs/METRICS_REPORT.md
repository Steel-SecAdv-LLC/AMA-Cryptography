...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 41,970 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 88 | 53,980 |
| Library total (Python + C + headers) | 118 | **95,950** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,460 |
| Tests (`tests/**/*.py`) | 275 | 125,458 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 784 | **465,903** |

**Library total (the figure that most closely tracks "library size"):
95,950 lines** across 118 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`465,903` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **20.6%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 95,950 | 20.6% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 125,458 | 26.9% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 241,137 | 51.8% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **465,903** | **100%** | sum of the scopes above |

Test code (26.9%) is roughly 1.3x the size of the library (20.6%) — i.e. the test-to-library ratio is roughly **1.31**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (51.8%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (72,834 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 270 |
| Syntactic `def test_` matches under `tests/**/*.py` | **6,557** |
