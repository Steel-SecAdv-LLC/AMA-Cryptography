...
| Tests (`tests/**/*.py`) | 165 | 68,025 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 595 | **327,441** |
...
**Whole-project total** (`327,441` lines across Python, C, headers,
...
| Tests | 68,025 | 20.8% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 168,819 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **327,441** | **100%** | sum of the scopes above |

Test code (20.8%) is roughly 0.8x the size of the library (26.8%) — i.e. the test-to-library ratio is roughly **0.78**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **3,905** |
...
The static count (3,905) and the dynamic collection count will differ.
