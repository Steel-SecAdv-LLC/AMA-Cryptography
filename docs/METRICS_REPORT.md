...
| **Whole project** (source + docs + config) | 720 | **402,781** |
...
**Whole-project total** (`402,781` lines across Python, C, headers,
...
| Everything else (remainder) | 208,254 | 51.7% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **402,781** | **100%** | sum of the scopes above |
...
vendored NIST ACVP and Wycheproof JSON corpora (71,559 lines of `*.json` alone) and by this repository's Markdown, not by config.
