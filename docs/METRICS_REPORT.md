...
| Tests (`tests/**/*.py`) | 260 | 108,887 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 754 | **431,189** |
...
**Whole-project total** (`431,189` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **21.8%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 94,028 | 21.8% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 108,887 | 25.3% | `tests/**/*.py` |
| Top-level Python | 1,431 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 224,970 | 52.2% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **431,189** | **100%** | sum of the scopes above |

Test code (25.3%) is roughly 1.2x the size of the library (21.8%) — i.e. the test-to-library ratio is roughly **1.16**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,846** |
