...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 35,750 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 105 | 51,535 |
| Library total (Python + C + headers) | 133 | **87,285** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,030 |
| Tests (`tests/**/*.py`) | 163 | 66,332 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 588 | **321,973** |

**Library total (the figure that most closely tracks "library size"):
87,285 lines** across 133 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`321,973` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 87,285 | 27.1% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 66,332 | 20.6% | `tests/**/*.py` |
...
| **Whole-project total** | **321,973** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **3,817** |
...
The static count (3,817) and the dynamic collection count will differ.
