...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 38,300 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 53,800 |
| Library total (Python + C + headers) | 137 | **92,100** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,260 |
| Tests (`tests/**/*.py`) | 199 | 85,442 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 655 | **371,661** |

**Library total (the figure that most closely tracks "library size"):
92,100 lines** across 137 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`371,661` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 92,100 | 24.8% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 85,442 | 23.0% | `tests/**/*.py` |
| Top-level Python | 1,260 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 190,986 | 51.4% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **371,661** | **100%** | sum of the scopes above |
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 196 |
| Syntactic `def test_` matches under `tests/**/*.py` | **4,647** |
