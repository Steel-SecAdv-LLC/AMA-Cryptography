...
| Tests (`tests/**/*.py`) | 218 | 94,209 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 681 | **386,113** |
...
**Whole-project total** (`386,113` lines across Python, C, headers,
...
| Tests | 94,209 | 24.4% | `tests/**/*.py` |
| Top-level Python | 1,292 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 195,821 | 50.7% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **386,113** | **100%** | sum of the scopes above |

Test code (24.4%) is roughly 1.0x the size of the library (24.1%) — i.e. the test-to-library ratio is roughly **1.01**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (50.7%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (70,741 lines of `*.json` alone) and by this repository's Markdown, not by config.
