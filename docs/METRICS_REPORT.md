...
| Tests (`tests/**/*.py`) | 207 | 89,261 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 666 | **379,201** |
...
**Whole-project total** (`379,201` lines across Python, C, headers,
...
| Tests | 89,261 | 23.5% | `tests/**/*.py` |
| Top-level Python | 1,264 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 194,205 | 51.2% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **379,201** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,766** |
