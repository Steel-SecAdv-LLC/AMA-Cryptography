...
| Tests (`tests/**/*.py`) | 164 | 66,633 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 592 | **323,492** |
...
**Whole-project total** (`323,492` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 87,658 | 27.1% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 66,633 | 20.6% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 166,298 | 51.4% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **323,492** | **100%** | sum of the scopes above |

Test code (20.6%) is roughly 0.8x the size of the library (27.1%) — i.e. the test-to-library ratio is roughly **0.76**, and that
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 161 |
| Syntactic `def test_` matches under `tests/**/*.py` | **3,833** |
...
The static count (3,833) and the dynamic collection count will differ.
