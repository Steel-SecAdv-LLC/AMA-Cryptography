...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 38,737 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 54,196 |
| Library total (Python + C + headers) | 137 | **92,933** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,292 |
| Tests (`tests/**/*.py`) | 213 | 90,567 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 694 | **385,638** |

**Library total (the figure that most closely tracks "library size"):
92,933 lines** across 137 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`385,638` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 92,933 | 24.1% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 90,567 | 23.5% | `tests/**/*.py` |
| Top-level Python | 1,292 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 198,973 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **385,638** | **100%** | sum of the scopes above |
