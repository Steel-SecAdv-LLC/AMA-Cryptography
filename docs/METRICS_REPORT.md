...
| Tests (`tests/**/*.py`) | 203 | 87,070 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 661 | **374,668** |
...
**Whole-project total** (`374,668` lines across Python, C, headers,
...
| Tests | 87,070 | 23.2% | `tests/**/*.py` |
| Top-level Python | 1,260 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 192,170 | 51.3% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **374,668** | **100%** | sum of the scopes above |

Test code (23.2%) is roughly 0.9x the size of the library (24.6%) — i.e. the test-to-library ratio is roughly **0.94**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,692** |
