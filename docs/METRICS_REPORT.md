...
| **Whole project** (source + docs + config) | 663 | **382,322** |
...
**Whole-project total** (`382,322` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 88,522 | 23.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 93,489 | 24.5% | `tests/**/*.py` |
| Top-level Python | 1,292 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 197,146 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **382,322** | **100%** | sum of the scopes above |
