...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 40,777 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 87 | 52,522 |
| Library total (Python + C + headers) | 117 | **93,299** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,423 |
| Tests (`tests/**/*.py`) | 251 | 104,575 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 741 | **419,381** |

**Library total (the figure that most closely tracks "library size"):
93,299 lines** across 117 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`419,381` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,299 | 22.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 104,575 | 24.9% | `tests/**/*.py` |
| Top-level Python | 1,423 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 218,211 | 52.0% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **419,381** | **100%** | sum of the scopes above |
...
vendored NIST ACVP and Wycheproof JSON corpora (71,944 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,623** |
| `test_*.c` files under `tests/c/` (ctest-registered) | 83 |
