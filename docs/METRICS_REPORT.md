...
| Tests (`tests/**/*.py`) | 271 | 122,106 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 777 | **457,675** |
...
**Whole-project total** (`457,675` lines across Python, C, headers,
...
| Tests | 122,106 | 26.7% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 236,404 | 51.7% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **457,675** | **100%** | sum of the scopes above |

Test code (26.7%) is roughly 1.3x the size of the library (20.9%) — i.e. the test-to-library ratio is roughly **1.27**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **6,397** |
