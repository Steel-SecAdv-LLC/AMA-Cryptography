...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 41,013 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 87 | 52,835 |
| Library total (Python + C + headers) | 117 | **93,848** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,423 |
| Tests (`tests/**/*.py`) | 252 | 105,271 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 744 | **422,123** |

**Library total (the figure that most closely tracks "library size"):
93,848 lines** across 117 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`422,123` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **22.2%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,848 | 22.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 105,271 | 24.9% | `tests/**/*.py` |
| Top-level Python | 1,423 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 219,708 | 52.0% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **422,123** | **100%** | sum of the scopes above |

Test code (24.9%) is roughly 1.1x the size of the library (22.2%) — i.e. the test-to-library ratio is roughly **1.12**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (52.0%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (72,000 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 248 |
| Syntactic `def test_` matches under `tests/**/*.py` | **5,658** |
| `test_*.c` files under `tests/c/` (ctest-registered) | 85 |
