...
| Tests (`tests/**/*.py`) | 184 | 78,583 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 633 | **355,075** |
...
**Whole-project total** (`355,075` lines across Python, C, headers,
...
| Tests | 78,583 | 22.1% | `tests/**/*.py` |
| Top-level Python | 1,060 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 182,769 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **355,075** | **100%** | sum of the scopes above |

Test code (22.1%) is roughly 0.9x the size of the library (25.6%) — i.e. the test-to-library ratio is roughly **0.87**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,379** |
