...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 40,751 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 87 | 52,354 |
| Library total (Python + C + headers) | 117 | **93,105** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,403 |
| Tests (`tests/**/*.py`) | 249 | 102,795 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 727 | **409,922** |

**Library total (the figure that most closely tracks "library size"):
93,105 lines** across 117 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`409,922` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **22.7%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,105 | 22.7% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 102,795 | 25.1% | `tests/**/*.py` |
| Top-level Python | 1,403 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 210,746 | 51.4% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **409,922** | **100%** | sum of the scopes above |

Test code (25.1%) is roughly 1.1x the size of the library (22.7%) — i.e. the test-to-library ratio is roughly **1.10**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (51.4%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (71,593 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 246 |
| Syntactic `def test_` matches under `tests/**/*.py` | **5,538** |
| `test_*.c` files under `tests/c/` (ctest-registered) | 82 |
