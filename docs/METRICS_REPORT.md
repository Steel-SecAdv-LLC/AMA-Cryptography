...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 38,232 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 53,483 |
| Library total (Python + C + headers) | 137 | **91,715** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,260 |
| Tests (`tests/**/*.py`) | 197 | 84,911 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 652 | **369,323** |

**Library total (the figure that most closely tracks "library size"):
91,715 lines** across 137 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`369,323` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **24.8%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 91,715 | 24.8% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 84,911 | 23.0% | `tests/**/*.py` |
| Top-level Python | 1,260 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 189,564 | 51.3% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **369,323** | **100%** | sum of the scopes above |
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 194 |
| Syntactic `def test_` matches under `tests/**/*.py` | **4,635** |
