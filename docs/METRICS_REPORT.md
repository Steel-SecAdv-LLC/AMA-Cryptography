...
| **Whole project** (source + docs + config) | 595 | **326,955** |
...
**Whole-project total** (`326,955` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 87,669 | 26.8% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 67,817 | 20.7% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 168,566 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **326,955** | **100%** | sum of the scopes above |

Test code (20.7%) is roughly 0.8x the size of the library (26.8%) — i.e. the test-to-library ratio is roughly **0.77**, and that
