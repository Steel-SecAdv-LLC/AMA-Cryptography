...
| Tests (`tests/**/*.py`) | 259 | 107,657 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 753 | **427,817** |
...
**Whole-project total** (`427,817` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 94,007 | 22.0% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 107,657 | 25.2% | `tests/**/*.py` |
| Top-level Python | 1,424 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 222,856 | 52.1% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **427,817** | **100%** | sum of the scopes above |

Test code (25.2%) is roughly 1.1x the size of the library (22.0%) — i.e. the test-to-library ratio is roughly **1.15**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,799** |
