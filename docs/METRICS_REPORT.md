...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 29 | 39,632 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 87 | 51,074 |
| Library total (Python + C + headers) | 116 | **90,706** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,403 |
| Tests (`tests/**/*.py`) | 245 | 100,549 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 720 | **402,700** |

**Library total (the figure that most closely tracks "library size"):
90,706 lines** across 116 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`402,700` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 90,706 | 22.5% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 100,549 | 25.0% | `tests/**/*.py` |
| Top-level Python | 1,403 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 208,169 | 51.7% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **402,700** | **100%** | sum of the scopes above |

Test code (25.0%) is roughly 1.1x the size of the library (22.5%) — i.e. the test-to-library ratio is roughly **1.11**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (51.7%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (71,550 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 242 |
| Syntactic `def test_` matches under `tests/**/*.py` | **5,439** |
