...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 38,506 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 54,181 |
| Library total (Python + C + headers) | 137 | **92,687** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,280 |
| Tests (`tests/**/*.py`) | 207 | 89,723 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 673 | **383,488** |

**Library total (the figure that most closely tracks "library size"):
92,687 lines** across 137 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`383,488` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **24.2%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 92,687 | 24.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 89,723 | 23.4% | `tests/**/*.py` |
| Top-level Python | 1,280 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 197,925 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **383,488** | **100%** | sum of the scopes above |

Test code (23.4%) is roughly 1.0x the size of the library (24.2%) — i.e. the test-to-library ratio is roughly **0.97**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (51.6%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (73,400 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,782** |
