...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 41,033 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 88 | 52,984 |
| Library total (Python + C + headers) | 118 | **94,017** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,424 |
| Tests (`tests/**/*.py`) | 259 | 107,496 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 753 | **427,762** |

**Library total (the figure that most closely tracks "library size"):
94,017 lines** across 118 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`427,762` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 94,017 | 22.0% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 107,496 | 25.1% | `tests/**/*.py` |
| Top-level Python | 1,424 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 222,952 | 52.1% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **427,762** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,796** |
