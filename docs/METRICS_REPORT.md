...
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 87 | 52,864 |
| Library total (Python + C + headers) | 117 | **93,877** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,424 |
| Tests (`tests/**/*.py`) | 252 | 105,271 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 744 | **422,282** |

**Library total (the figure that most closely tracks "library size"):
93,877 lines** across 117 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`422,282` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,877 | 22.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 105,271 | 24.9% | `tests/**/*.py` |
| Top-level Python | 1,424 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 219,837 | 52.1% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **422,282** | **100%** | sum of the scopes above |
