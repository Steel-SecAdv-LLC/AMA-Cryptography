...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 29 | 37,531 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 53,188 |
| Library total (Python + C + headers) | 138 | **90,719** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,060 |
| Tests (`tests/**/*.py`) | 184 | 78,310 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 633 | **354,607** |

**Library total (the figure that most closely tracks "library size"):
90,719 lines** across 138 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`354,607` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 90,719 | 25.6% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 78,310 | 22.1% | `tests/**/*.py` |
| Top-level Python | 1,060 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 182,645 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **354,607** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,374** |
