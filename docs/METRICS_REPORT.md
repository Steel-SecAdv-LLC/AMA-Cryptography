...
| Tests (`tests/**/*.py`) | 275 | 125,092 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 781 | **463,329** |
...
**Whole-project total** (`463,329` lines across Python, C, headers,
...
| Tests | 125,092 | 27.0% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 239,067 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **463,329** | **100%** | sum of the scopes above |

Test code (27.0%) is roughly 1.3x the size of the library (20.7%) — i.e. the test-to-library ratio is roughly **1.31**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **6,537** |
