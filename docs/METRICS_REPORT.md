...
| **Whole project** (source + docs + config) | 761 | **436,078** |
...
**Whole-project total** (`436,078` lines across Python, C, headers,
...
| Tests | 108,205 | 24.8% | `tests/**/*.py` |
| Top-level Python | 1,431 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 230,541 | 52.9% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **436,078** | **100%** | sum of the scopes above |

Test code (24.8%) is roughly 1.2x the size of the library (21.6%) — i.e. the test-to-library ratio is roughly **1.15**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (52.9%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (73,727 lines of `*.json` alone) and by this repository's Markdown, not by config.
