...
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 53,540 |
| Library total (Python + C + headers) | 137 | **91,697** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,260 |
| Tests (`tests/**/*.py`) | 193 | 83,143 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 645 | **365,692** |

**Library total (the figure that most closely tracks "library size"):
91,697 lines** across 137 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`365,692` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 91,697 | 25.1% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 83,143 | 22.7% | `tests/**/*.py` |
| Top-level Python | 1,260 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 187,719 | 51.3% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **365,692** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,549** |
