...
| Tests (`tests/**/*.py`) | 251 | 104,644 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 741 | **419,674** |
...
**Whole-project total** (`419,674` lines across Python, C, headers,
...
| Tests | 104,644 | 24.9% | `tests/**/*.py` |
| Top-level Python | 1,423 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 218,436 | 52.0% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **419,674** | **100%** | sum of the scopes above |
...
vendored NIST ACVP and Wycheproof JSON corpora (71,954 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,629** |
