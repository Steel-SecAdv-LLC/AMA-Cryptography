...
| Tests (`tests/**/*.py`) | 260 | 108,061 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 756 | **429,185** |
...
**Whole-project total** (`429,185` lines across Python, C, headers,
...
| Tests | 108,061 | 25.2% | `tests/**/*.py` |
| Top-level Python | 1,431 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 223,752 | 52.1% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **429,185** | **100%** | sum of the scopes above |

Test code (25.2%) is roughly 1.1x the size of the library (21.9%) — i.e. the test-to-library ratio is roughly **1.15**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,821** |
