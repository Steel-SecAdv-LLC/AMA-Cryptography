...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 35,504 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 105 | 51,488 |
| Library total (Python + C + headers) | 133 | **86,992** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,030 |
| Tests (`tests/**/*.py`) | 161 | 65,216 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 585 | **320,024** |

**Library total (the figure that most closely tracks "library size"):
86,992 lines** across 133 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`320,024` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 86,992 | 27.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 65,216 | 20.4% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 164,913 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **320,024** | **100%** | sum of the scopes above |

Test code (20.4%) is roughly 0.7x the size of the library (27.2%) — i.e. the test-to-library ratio is roughly **0.75**, and that
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 158 |
| Syntactic `def test_` matches under `tests/**/*.py` | **3,770** |
...
The static count (3,770) and the dynamic collection count will differ.
