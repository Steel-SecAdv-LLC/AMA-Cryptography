...
| **Whole project** (source + docs + config) | 668 | **379,396** |
...
**Whole-project total** (`379,396` lines across Python, C, headers,
...
| Tests | 89,340 | 23.5% | `tests/**/*.py` |
| Top-level Python | 1,264 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 194,321 | 51.2% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **379,396** | **100%** | sum of the scopes above |
