...
| Tests (`tests/**/*.py`) | 273 | 123,131 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 775 | **458,885** |
...
**Whole-project total** (`458,885` lines across Python, C, headers,
...
| Tests | 123,131 | 26.8% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 236,588 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **458,885** | **100%** | sum of the scopes above |

Test code (26.8%) is roughly 1.3x the size of the library (20.9%) — i.e. the test-to-library ratio is roughly **1.29**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (51.6%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (72,829 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 268 |
| Syntactic `def test_` matches under `tests/**/*.py` | **6,424** |
