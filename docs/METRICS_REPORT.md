...
| Tests (`tests/**/*.py`) | 260 | 107,641 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 756 | **428,318** |
...
**Whole-project total** (`428,318` lines across Python, C, headers,
...
| Tests | 107,641 | 25.1% | `tests/**/*.py` |
| Top-level Python | 1,424 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 223,363 | 52.1% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **428,318** | **100%** | sum of the scopes above |
...
vendored NIST ACVP and Wycheproof JSON corpora (72,411 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,810** |
