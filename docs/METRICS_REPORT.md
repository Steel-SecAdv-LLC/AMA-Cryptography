...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 41,949 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 88 | 53,859 |
| Library total (Python + C + headers) | 118 | **95,808** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,460 |
| Tests (`tests/**/*.py`) | 272 | 123,023 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 778 | **458,692** |

**Library total (the figure that most closely tracks "library size"):
95,808 lines** across 118 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`458,692` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 95,808 | 20.9% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 123,023 | 26.8% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 236,503 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **458,692** | **100%** | sum of the scopes above |
...
vendored NIST ACVP and Wycheproof JSON corpora (72,828 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 267 |
| Syntactic `def test_` matches under `tests/**/*.py` | **6,416** |
