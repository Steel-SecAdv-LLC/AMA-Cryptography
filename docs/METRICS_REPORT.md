...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 29 | 37,602 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 53,188 |
| Library total (Python + C + headers) | 138 | **90,790** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,060 |
| Tests (`tests/**/*.py`) | 184 | 78,420 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 633 | **354,862** |

**Library total (the figure that most closely tracks "library size"):
90,790 lines** across 138 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`354,862` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **25.6%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 90,790 | 25.6% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 78,420 | 22.1% | `tests/**/*.py` |
| Top-level Python | 1,060 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 182,719 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **354,862** | **100%** | sum of the scopes above |

Test code (22.1%) is roughly 0.9x the size of the library (25.6%) — i.e. the test-to-library ratio is roughly **0.86**, and that
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 181 |
| Syntactic `def test_` matches under `tests/**/*.py` | **4,376** |
| `test_*.c` files under `tests/c/` (ctest-registered) | 60 |
