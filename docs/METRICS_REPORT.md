...
| Tests (`tests/**/*.py`) | 213 | 90,611 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 684 | **385,174** |
...
**Whole-project total** (`385,174` lines across Python, C, headers,
...
| Tests | 90,611 | 23.5% | `tests/**/*.py` |
| Top-level Python | 1,292 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 198,465 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **385,174** | **100%** | sum of the scopes above |

Test code (23.5%) is roughly 1.0x the size of the library (24.1%) — i.e. the test-to-library ratio is roughly **0.98**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,813** |
