...
| Tests (`tests/**/*.py`) | 165 | 66,955 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 594 | **324,509** |
...
**Whole-project total** (`324,509` lines across Python, C, headers,
...
| Tests | 66,955 | 20.6% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 166,993 | 51.5% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **324,509** | **100%** | sum of the scopes above |

Test code (20.6%) is roughly 0.8x the size of the library (27.0%) — i.e. the test-to-library ratio is roughly **0.76**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **3,857** |
...
The static count (3,857) and the dynamic collection count will differ.
