...
| Tests (`tests/**/*.py`) | 172 | 70,618 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 613 | **337,631** |
...
**Whole-project total** (`337,631` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **26.4%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 88,989 | 26.4% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 70,618 | 20.9% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 175,121 | 51.9% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **337,631** | **100%** | sum of the scopes above |

Test code (20.9%) is roughly 0.8x the size of the library (26.4%) — i.e. the test-to-library ratio is roughly **0.79**, and that
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 169 |
| Syntactic `def test_` matches under `tests/**/*.py` | **4,042** |
...
The static count (4,042) and the dynamic collection count will differ.
