...
| Tests (`tests/**/*.py`) | 207 | 89,210 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 666 | **379,114** |
...
**Whole-project total** (`379,114` lines across Python, C, headers,
...
| Tests | 89,210 | 23.5% | `tests/**/*.py` |
| Top-level Python | 1,264 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 194,169 | 51.2% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **379,114** | **100%** | sum of the scopes above |

Test code (23.5%) is roughly 1.0x the size of the library (24.4%) — i.e. the test-to-library ratio is roughly **0.96**, and that
counts only `tests/**/*.py`; the C test suite under `tests/c/` lands
in the remainder row. The remainder (51.2%) is dominated by the
vendored NIST ACVP and Wycheproof JSON corpora (70,692 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,764** |
