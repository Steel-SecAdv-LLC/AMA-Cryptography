...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 40,774 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 87 | 52,524 |
| Library total (Python + C + headers) | 117 | **93,298** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,423 |
| Tests (`tests/**/*.py`) | 251 | 104,575 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 741 | **419,380** |

**Library total (the figure that most closely tracks "library size"):
93,298 lines** across 117 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`419,380` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,298 | 22.2% | `ama_cryptography/` + `src/c/` + `include/` |
...
| **Whole-project total** | **419,380** | **100%** | sum of the scopes above |
