...
| **Whole project** (source + docs + config) | 729 | **411,184** |
...
**Whole-project total** (`411,184` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,105 | 22.6% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 102,795 | 25.0% | `tests/**/*.py` |
| Top-level Python | 1,403 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 212,008 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **411,184** | **100%** | sum of the scopes above |

Test code (25.0%) is roughly 1.1x the size of the library (22.6%) — i.e. the test-to-library ratio is roughly **1.10**, and that
