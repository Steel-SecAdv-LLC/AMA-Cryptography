...
| Tests (`tests/**/*.py`) | 219 | 93,492 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 663 | **382,307** |
...
**Whole-project total** (`382,307` lines across Python, C, headers,
...
| Tests | 93,492 | 24.5% | `tests/**/*.py` |
| Top-level Python | 1,292 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 197,128 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **382,307** | **100%** | sum of the scopes above |
