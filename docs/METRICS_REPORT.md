...
| Tests (`tests/**/*.py`) | 218 | 94,230 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 691 | **389,736** |
...
**Whole-project total** (`389,736` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **23.8%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 92,918 | 23.8% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 94,230 | 24.2% | `tests/**/*.py` |
| Top-level Python | 1,292 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 199,423 | 51.2% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **389,736** | **100%** | sum of the scopes above |

Test code (24.2%) is roughly 1.0x the size of the library (23.8%) — i.e. the test-to-library ratio is roughly **1.01**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **5,127** |
