...
| Tests (`tests/**/*.py`) | 271 | 122,930 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 777 | **458,569** |
...
**Whole-project total** (`458,569` lines across Python, C, headers,
...
| Tests | 122,930 | 26.8% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 236,474 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **458,569** | **100%** | sum of the scopes above |
...
| Syntactic `def test_` matches under `tests/**/*.py` | **6,413** |
