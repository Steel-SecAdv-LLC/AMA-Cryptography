...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 30 | 41,953 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 88 | 53,859 |
| Library total (Python + C + headers) | 118 | **95,812** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,460 |
| Tests (`tests/**/*.py`) | 274 | 124,628 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,898 |
| **Whole project** (source + docs + config) | 779 | **462,442** |

**Library total (the figure that most closely tracks "library size"):
95,812 lines** across 118 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`462,442` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **20.7%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 95,812 | 20.7% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 124,628 | 26.9% | `tests/**/*.py` |
| Top-level Python | 1,460 | 0.3% | `*.py` at repo root |
| Cython | 1,898 | 0.4% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 238,644 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **462,442** | **100%** | sum of the scopes above |

Test code (26.9%) is roughly 1.3x the size of the library (20.7%) — i.e. the test-to-library ratio is roughly **1.30**, and that
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 269 |
| Syntactic `def test_` matches under `tests/**/*.py` | **6,519** |
