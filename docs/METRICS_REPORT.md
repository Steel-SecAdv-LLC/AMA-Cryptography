...
| **Whole project** (source + docs + config) | 727 | **410,343** |
...
**Whole-project total** (`410,343` lines across Python, C, headers,
...
| Tests | 102,800 | 25.1% | `tests/**/*.py` |
| Top-level Python | 1,403 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 211,002 | 51.4% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **410,343** | **100%** | sum of the scopes above |
