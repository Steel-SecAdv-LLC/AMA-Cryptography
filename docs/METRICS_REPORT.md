...
| Tests (`tests/**/*.py`) | 169 | 69,147 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 604 | **332,608** |
...
**Whole-project total** (`332,608` lines across Python, C, headers,
...
| Tests | 69,147 | 20.8% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 171,681 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **332,608** | **100%** | sum of the scopes above |
...
| Scope | Count |
|-------|------:|
| Python test files under `tests/` matching the static regex | 166 |
| Syntactic `def test_` matches under `tests/**/*.py` | **3,971** |
...
The static count (3,971) and the dynamic collection count will differ.
