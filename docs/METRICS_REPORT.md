...
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 105 | 51,535 |
| Library total (Python + C + headers) | 133 | **87,334** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,030 |
| Tests (`tests/**/*.py`) | 163 | 66,420 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 588 | **322,110** |

**Library total (the figure that most closely tracks "library size"):
87,334 lines** across 133 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`322,110` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 87,334 | 27.1% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 66,420 | 20.6% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 165,453 | 51.4% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **322,110** | **100%** | sum of the scopes above |
...
vendored NIST ACVP and Wycheproof JSON corpora (69,553 lines of `*.json` alone) and by this repository's Markdown, not by config.
...
| Syntactic `def test_` matches under `tests/**/*.py` | **3,820** |
...
The static count (3,820) and the dynamic collection count will differ.
