...
| Tests (`tests/**/*.py`) | 172 | 70,718 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 613 | **337,731** |
...
**Whole-project total** (`337,731` lines across Python, C, headers,
...
| **Whole-project total** | **337,731** | **100%** | sum of the scopes above |

Test code (20.9%) is roughly 0.8x the size of the library (26.3%) — i.e. the test-to-library ratio is roughly **0.79**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **4,045** |
...
The static count (4,045) and the dynamic collection count will differ.
