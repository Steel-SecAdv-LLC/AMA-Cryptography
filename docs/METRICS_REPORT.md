...
The gap between "library" and "whole project" is informative: only **22.6%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 93,278 | 22.6% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 102,800 | 24.9% | `tests/**/*.py` |
| Top-level Python | 1,423 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 213,711 | 51.7% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **413,085** | **100%** | sum of the scopes above |

Test code (24.9%) is roughly 1.1x the size of the library (22.6%) — i.e. the test-to-library ratio is roughly **1.10**, and that
