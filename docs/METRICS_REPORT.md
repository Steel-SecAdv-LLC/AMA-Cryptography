...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 38,252 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 109 | 53,470 |
| Library total (Python + C + headers) | 137 | **91,722** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,260 |
| Tests (`tests/**/*.py`) | 197 | 84,921 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 652 | **369,483** |
...
**Whole-project total** (`369,483` lines across Python, C, headers,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 91,722 | 24.8% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 84,921 | 23.0% | `tests/**/*.py` |
| Top-level Python | 1,260 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.5% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 189,707 | 51.3% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **369,483** | **100%** | sum of the scopes above |
