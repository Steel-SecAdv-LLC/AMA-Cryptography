...
| Scope | Files | Lines |
|-------|------:|------:|
| Library Python (`ama_cryptography/*.py`) | 28 | 35,210 |
| Native C (`src/c/**/*.c`, `include/**/*.h`) | 105 | 51,448 |
| Library total (Python + C + headers) | 133 | **86,658** |
| Top-level Python (monitors, benchmarks, demos) | 2 | 1,030 |
| Tests (`tests/**/*.py`) | 160 | 64,507 |
| Cython (`*.pyx`, `*.pxd`) | 7 | 1,873 |
| **Whole project** (source + docs + config) | 584 | **318,127** |

**Library total (the figure that most closely tracks "library size"):
86,658 lines** across 133 files under `ama_cryptography/`, `src/c/`,
and `include/`. This supersedes any "11,246 LoC" claim that may have
appeared externally.

**Whole-project total** (`318,127` lines across Python, C, headers,
...
The gap between "library" and "whole project" is informative: only **27.2%** of the repository is library code. The rest is tests, docs,
...
|--------------------------------------|---------:|-----------:|---------------------------------------------------------|
| Library (Python + C + headers) | 86,658 | 27.2% | `ama_cryptography/` + `src/c/` + `include/` |
| Tests | 64,507 | 20.3% | `tests/**/*.py` |
| Top-level Python | 1,030 | 0.3% | `*.py` at repo root |
| Cython | 1,873 | 0.6% | `*.pyx` + `*.pxd` |
| Everything else (remainder) | 164,059 | 51.6% | `*.md`, `*.yml`, `*.toml`, `*.json`, CMake, Makefile, plus `.c`/`.h`/`.py` outside the scopes above (`tests/c/`, `fuzz/`, `tools/`, `benchmarks/`, `examples/`) |
| **Whole-project total** | **318,127** | **100%** | sum of the scopes above |

Test code (20.3%) is roughly 0.7x the size of the library (27.2%) — i.e. the test-to-library ratio is roughly **0.74**, and that
...
| Syntactic `def test_` matches under `tests/**/*.py` | **3,740** |
...
The static count (3,740) and the dynamic collection count will differ.
