...
# docs/audit/ledger.tsv and a retained log under docs/audit/logs/phaseA/.
#
# Host-independent: session-local binaries are supplied through environment
# variables rather than hard-coded paths, so the script re-runs on any host.
#
#   AUDIT_CODEQL   path to the CodeQL CLI binary (default: codeql on PATH)
#   AUDIT_COSIGN   path to cosign (default: cosign on PATH)
#   AUDIT_VENV     virtualenv whose bin/ is prepended to PATH
#                  (default: /opt/ama-venv)
#
...
P="phaseA"
export PATH="${AUDIT_VENV:-/opt/ama-venv}/bin:$PATH"
CODEQL="${AUDIT_CODEQL:-codeql}"
COSIGN="${AUDIT_COSIGN:-cosign}"
...
$R A-03 "host CPU, kernel, memory, disk" $P/host -- \
  bash -c 'uname -a; cat /etc/os-release | head -2; lscpu | grep -E "Model name|^CPU\(s\)|Flags|Hypervisor|Model:"; free -g | head -2; df -h . | tail -1'
$R A-04 "cmake versions (system and pip shim)" $P/cmake -- \
  bash -c '/usr/bin/cmake --version | head -1; cmake --version | head -1; which cmake'
$R A-05 "C compilers: gcc, clang, aarch64 cross, 32-bit x86" $P/compilers -- \
  bash -c 'gcc --version | head -1; clang --version | head -1; aarch64-linux-gnu-gcc-13 --version | head -1; gcc -m32 -x c -o /dev/null - <<<"int main(void){return 0;}" && echo "gcc -m32 ok"; clang -m32 -x c -o /dev/null - <<<"int main(void){return 0;}" && echo "clang -m32 ok"'
$R A-06 "qemu-user aarch64 and SVE vector lengths reachable" $P/qemu-sve -- \
  bash -c 'qemu-aarch64-static --version | head -1; t=$(mktemp -d); printf "#include <arm_sve.h>\n#include <stdio.h>\nint main(void){printf(\"VL=%%lu bytes\\\\n\",(unsigned long)svcntb());return 0;}\n" > $t/sve.c; aarch64-linux-gnu-gcc-13 -march=armv9-a+sve2 -static $t/sve.c -o $t/sve && for vq in 1 2 4 8 16; do printf "sve-max-vq=%s (default VL): " $vq; qemu-aarch64-static -cpu max,sve-max-vq=$vq $t/sve; done; for vq in 8 16; do printf "sve-max-vq=%s,sve-default-vector-length=-1: " $vq; qemu-aarch64-static -cpu max,sve-max-vq=$vq,sve-default-vector-length=-1 $t/sve; done; printf "cortex-a53 (no SVE): "; qemu-aarch64-static -cpu cortex-a53 $t/sve; echo "exit=$?"'
...
$R A-14 "CodeQL CLI (bundle downloaded this session; path via AUDIT_CODEQL)" $P/codeql -- \
  bash -c "$CODEQL version 2>/dev/null | grep -v JAVA_TOOL; $CODEQL resolve languages 2>/dev/null | grep -E '^(python|cpp|actions) ' | cut -d' ' -f1"
...
$R A-16 "softhsm2 + PyKCS11 + pkcs11-tool" $P/softhsm -- \
  bash -c 'softhsm2-util --version; ls -la /usr/lib/softhsm/libsofthsm2.so; python3 -c "import PyKCS11; print(\"PyKCS11\", PyKCS11.__version__ if hasattr(PyKCS11, \"__version__\") else \"ok\")"; pkcs11-tool --version 2>&1 | head -1'
...
$R A-23 "x86 ISA extensions present on this host (lscpu flags)" $P/hw-flags -- \
  bash -c 'for f in avx2 avx512f avx512vl avx512bw avx512ifma vaes vpclmulqdq gfni sha_ni avx512_fp16 amx_tile sve sve2; do printf "%-12s " $f; lscpu | grep -q -w "$f" && echo present || echo ABSENT; done; echo "os: $(uname -s) (no Windows, no macOS, no real HSM, no AArch64 silicon)"'
$R A-24 "Release C build + ctest (baseline, this host)" $P/ctest-release -- \
  bash -c 'cd build-release && ctest -N | tail -1 && ctest -j4 2>&1 | grep -E "tests passed|Total Test time|Skipped"'
