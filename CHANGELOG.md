...
> **Not yet released.** `v5.0.0` is not yet tagged; the newest release tag is
> `v4.0.0`. The release commit dates this heading and removes this note, and
> `tools/check_release_state.py` refuses a `v5.0.0` tag until both are done.
>
> **The long form is in git history.** This section is a digest. Until
> 2026-09-24 the file carried a separate `[Unreleased]` section and a
> `[5.0.0] - 2026-09-10` section holding about thirty dated pass-by-pass
> journals (roughly 9,650 lines) with every measurement, mutation run, retraction
> and dead end. Commit `d8b1fca1` is the last one that holds that long form:
> `git show d8b1fca1:CHANGELOG.md` recovers it (lines 20–9699). Each item below
> cites the commit or commits that made the change, so the reasoning behind any
> item can be read with `git show <sha>`.

> **Dated figures are dated.** A measurement quoted below carries the date and
> host it was taken on, and describes the tree as it stood then. The current
> counts and measurements live in the files the gates read
> (`docs/METRICS_REPORT.md`, `benchmarks/baseline.json`,
> `benchmarks/published-benchmarks.json`, the compliance attestations). This
> section is itself read by a gate: while its heading reads `Unreleased`,
> `tools/check_documented_counts.py` checks the entry-point, C-suite and
> source-inventory counts it states against the tree, so it states as few as
> it can. The long-form journal at `d8b1fca1` is a record: no gate reads a
> pass entry as a current property.
...
The table's pointers into the narrative ("see *Completion pass 2* below")
resolve in the long-form journal at `d8b1fca1`; the corresponding digest items
are under **Changed** below.

### Wire-format and ABI changes
...
(INVARIANT-42) now expects major version 5. The C API itself changed.
...
Formats that changed on the wire or on disk. Each item states what an existing
artefact does under 5.0.0 and what to do about it.

- **ML-DSA-65 signs on the FIPS 204 §5.2 external interface.**
  `dilithium_sign` / `dilithium_verify`, `crypto_api.sign` / `.verify` for
  ML-DSA-65 and `native_ml_dsa_sign` / `_verify` use the empty context by
  default; 4.x called Algorithm 7 (`Sign_internal`) on the raw message, so 4.x
  signatures do not verify under 5.0.0 and vice versa. *Migration:* re-sign
  with 5.0.0; a conforming third-party verifier now accepts the output.
  (`e8b9c8c1`, `0cf4fc0d`)
- **Hybrid signatures are format v2.** Both halves sign the message bound to
  `AMA-Cryptography/hybrid-sig/v2/Ed25519+ML-DSA-65` (Ed25519 over a
  `0x00 || len || domain || M` wrapper, ML-DSA-65 with the same string as its
  context), so neither half is a standalone signature any more. v1 signatures do
  not verify. *Migration:* re-sign. (`e8b9c8c1`)
- **`AMA_ALG_HYBRID` in the C API is Ed25519 + ML-DSA-65**, Ed25519 part
  first, in the same v2 format (`AMA_HYBRID_PUBLIC_KEY_BYTES` = 1,984,
  `AMA_HYBRID_SECRET_KEY_BYTES` = 4,096, `AMA_HYBRID_SIGNATURE_BYTES` = 3,373);
  in 4.x it was an alias for ML-DSA-65 alone. *Migration:* resize buffers from
  the macros. (`e8b9c8c1`)
- **SLH-DSA's legacy entry points are FIPS 205 §10.2.** `ama_sphincs_sign` /
  `ama_sphincs_verify` and the generic `ama_sign` / `ama_verify` for
  `AMA_ALG_SPHINCS_256F` apply the §10.2 wrapper with the empty context; 4.x
  signed the raw message (§9). *Migration:* re-sign; `ama_slhdsa_sign_addrnd`
  replaces `ama_slhdsa_sign_internal` for callers that supply `addrnd`.
  (`9b860868`)
- **A `crypto_api` package is signed over a transcript of the whole package**
  (INVARIANT-52): an injective, length-prefixed encoding of every field and of
  the presence of every optional layer, plus
  `metadata["kem_shared_secret_commitment"]`; a `crypto_api` package created
  by 4.x does not verify. The legacy surface now mints `SIGNATURE_FORMAT_V3`,
  which signs and MACs the whole record, and still verifies V1 and V2.
  *Migration:* re-create `crypto_api` packages. (`9b860868`, `0cf4fc0d`)
- **The integrity artefact is v3 and the package digest is v2.** The signature
  covers `AMA-integrity-signature-v3` (native library plus the six binding
  extensions), and the package digest is length-prefixed under
  `AMA-package-digest-v2\0`, so no pre-5.0.0 artefact verifies. The artefact
  is a build output: every build signs its own (see **Changed**). *Migration:*
  rebuild; `AMA_BUILD_PIPELINE=1 python -m ama_cryptography.integrity --update --sign`
  repairs a developer tree. (`3f645c1d`, `fe8b8ab4`, `514c0942`)
- **Argon2id refuses out-of-domain parameters** (`p = 0`, `p > 255`, `t = 0`,
  `m < 8p`, an empty salt) instead of silently clamping them and returning a
  tag no conforming implementation reproduces. *Migration:* pass valid
  parameters; a tag stored from a clamped call must be re-derived. (`e8b9c8c1`)
- **The shared library exports only its declared `ama_*` ABI** on ELF (version
  script), macOS (`-unexported_symbols_list` generated from the same script) and
  MinGW (a generated `.def`), so a consumer resolving an internal helper by name
  no longer links. *Migration:* use the public headers. (`e8b9c8c1`,
  `01a4c823`, `ebaae183`, `549980b5`)
- **`ama_secure_alloc()` is page-granular** (`mmap` / `VirtualAlloc`), so
  releasing one buffer no longer unlocks its neighbours. No API change.
  (`e8b9c8c1`)
- **The dispatch auto-tune cache fingerprint is `v2`**; a v1 cache is ignored
  and rebuilt, not read back. (`75881a55`)

### Security

Signing, verification and key handling:

- **Two signing oracles closed.** The KEM layer of a `crypto_api` package
  verified nothing (the Kyber secret key and `kem_shared_secret` were outside
  the signature); the package now signs a commitment to the secret. And the
  ML-DSA internal interface shipped, so `ama_ml_dsa_sign(0x00‖0x01‖"x"‖M)`
  verified under `ama_ml_dsa_verify_ctx(M, "x")`; it is test-only now
  (INVARIANT-50). (`0cf4fc0d`)
- **The FIPS 205 §9 internal interface no longer ships** (INVARIANT-50): §9
  and §10.2 signatures cross-verified under one key, which made the legacy and
  generic APIs a signing oracle for pure signatures on attacker-chosen
  `(ctx, M)`. (`9b860868`)
- **Ed25519 rejects small-order public keys and R halves** (INVARIANT-48). The
  cofactorless verifier accepted an identity public key, giving one key-free
  signature valid for every message; all eight small-order points admitted
  forgeries (12/12 universal and 8/8 per-key before, 0 after). (`9b860868`)
- **An Ed25519 signer derives its own public half** (INVARIANT-51): two
  signatures over one message under two different stored `A` halves leak the
  private scalar. The check costs one fixed-base multiplication per signature;
  `ama_ed25519_sign_expanded` pays it once at load (see **Added**).
  (`9b860868`, `0fb68b4f`, `84ad90d2`, `24341f89`)
- **`ama_ed25519_batch_verify` agrees with single verify on non-canonical R**
  (glance row 14): at `count >= 4` the batch path decoded R instead of
  re-encoding it and accepted what single verify rejected. (`8157a86d`)
- **Ed25519 decoding** rejects `x = 0` with the sign bit set (RFC 8032 §5.1.3,
  row 2) and `y >= p` (INVARIANT-38); pinned on the non-verify decoders by
  `tests/c/test_ed25519_decode_x_zero.c`. (`839b66b4`)
- **`ama_verify(AMA_ALG_ED25519)` requires exactly 64 signature bytes**; it
  accepted a valid signature followed by trailing bytes. (`150a99c3`)
- **A failing Ed25519 signer zeroes its output** on every error exit (an
  oversized `message_len`, an allocation failure) instead of leaving a previous
  signature in a reused buffer. (`c126037c`)
- **FROST nonce pairs are single-use and aggregation verifies every share**
  (INVARIANT-49). Reusing one nonce pair over three messages recovered the
  participant's long-term share; round 2 now consumes the pair on every exit,
  and `ama_frost_aggregate` verifies each share, names the culprit and checks
  the result against RFC 8032. Shares must be canonical (`z < L`), and an
  undecodable commitment is attributed. (`9b860868`, `f43af32c`, `8ddf6062`)
- **secp256k1 signed under an all-zero key on MSVC.** An unsequenced
  `f(&d) | g(&d)` predicate let MSVC evaluate right to left; four sites in
  `ama_secp256k1.c` and `ama_nistp.c` were split, and
  `tests/test_unsequenced_predicate_gate.py` rejects the shape. (`28b70b51`)
- **secp256k1 point validation is one function at both public entries**:
  non-canonical and off-curve coordinates are rejected, not reduced.
  (`611a41f5`)
- **The agent-binding authority key is an input to the derivation**
  (INVARIANT-30): restricted bindings' HKDF output and signature context take an
  `HMAC-SHA3-256(K_auth, …)` binder, so an in-process caller without `K_auth`
  cannot compute them by calling `ama_hkdf` directly. (`9b860868`)

Constant-time and secret hygiene:

- **A live ECDSA timing leak introduced by codegen.** At `-O3` clang turned
  secp256k1 `sc_mont_mul`'s mask into a branch keyed on the RFC 6979 nonce
  (a 9,424-instruction key-dependent spread under callgrind, clang 18.1.3,
  2026-08-19) and did the same in
  `nistp_mont_mul`; every secret mask now passes through
  `ama_ct_value_barrier_u64`. The constant-time gates had been measuring an
  unoptimised library, and now refuse one. (`e52281c3`)
- **Secret-dependent branches found by the new secret-taint mode**
  (`check_ghash_constant_time.py --taint`) are removed: the X25519 all-zero
  check, the safegcd product re-check, secp256k1 zero/infinity branches, an
  AES-GCM verify-mask `cmov`, and ML-DSA's early-exit norm checks. Remaining
  public-verdict branches are declassified in
  `src/c/internal/ama_ct_declassify.h`. (`e8b9c8c1`)
- **AEAD decrypt returns are selected by mask** in ChaCha20-Poly1305 and all
  four AES-256-GCM paths, so accept and reject retire identical instruction
  counts (row 13); the AES-NI and VAES tag masks go through the value barrier.
  (`7bc7e587`, `94e4bd68`, `150a99c3`)
- **Key material off the dead stack** (INVARIANT-6): hardware AES-GCM left all
  fifteen round keys in compiler-created stack slots and ChaCha20 left its
  state, fixed with the new `ama_secure_stack_wipe()`; Ed25519's secret entry
  points wipe 8 KiB; Ascon-AEAD128, which had no wipe, runs in noinline workers
  followed by one. (`e8b9c8c1`, `8ddf6062`)
- **Stack scrubs added where secrets were staged**: NEON AES key schedule and
  GHASH accumulator, SIMD AES-GCM partial blocks, SVE2 Kyber and Dilithium
  scratch, AVX2/NEON ML-DSA NTT staging, the scalar Kyber CBD seed, the SPHINCS
  NEON SHA-256 schedule, the Argon2 G kernels, secp256k1 sign temporaries, and
  Poly1305's clamped `r`. (`2dcc282`, `a4cbf55`, `b7e9fa1`, `8157a86d`,
  `611a41f5`, `c4c83479`, `17e5d79d`, `7766faaf`)
- **`ama_secure_mlock()` applies `MADV_DONTDUMP`** (strict ISO mode had
  compiled it out, and unaligned addresses were refused), and the advice now
  survives an `mlock` failure. (`17e5d79d`, `150a99c3`)
- **Python-side wipes and copies.** `secure_memzero` / `secure_mlock` sized a
  memoryview in items, not bytes, and left 24 of 32 bytes of an
  `array('I')` secret; `Ed25519SigningKey` no longer makes un-wipeable `bytes`
  copies of the seed; `SecureChannelInitiator` clears the shared secret on
  every failed handshake; session keys no longer survive TTL expiry or leak
  through `asdict`. (`f8873ae8`, `c126037c`, `9b860868`)

Module integrity (INVARIANT-39/40/17):

- **A failed POST fails the import and the ERROR state inhibits output** on
  every surface (row 1); the executed bytecode must match the signed source
  (INVARIANT-40, row 10); every asymmetric keygen runs a pairwise consistency
  test (INVARIANT-41, row 4); the declared ctypes ABI is checked against the
  header and the loaded library's major version (INVARIANT-42). (`7766faaf`)
- **The native library is verified before it is mapped** (row 6): candidates
  are hashed first and loaded through `/proc/self/fd` on Linux.
  `AMA_CRYPTO_LIB_PATH` now relocates the signed library and can no longer
  substitute one: a differing object is refused before mapping, with no
  fallback (INVARIANT-7), and the reason is in
  `native_backend_diagnostics()["override_refusal"]`. (`7766faaf`, `d9cdb3cc`)
- **An environment variable no longer buys execution.** `AMA_BUILD_PIPELINE=1`
  demoted the pre-load refusal, and later let the read-only
  `integrity --verify` map a tampered library; signing scope now requires the
  process to be the signer with signing intent, revoked in secure-execution
  mode (rows 21, 6). (`2ac912e9`, `c4c83479`, `0cf4fc0d`)
- **Binding extensions are digest-bound and refused before they execute**
  (row 7): a tampered extension is refused ahead of its module-init code, and
  the package's own binding probes no longer map a library the pre-load check
  had just refused (their `DT_NEEDED` resolved it through `$ORIGIN`).
  (`3f645c1d`, `2ac912e9`, `c8081aff`)
- **The integrity artefact cannot be poisoned through the import system**: it
  is parsed from source text (`ama_cryptography/_artefact_source.py`) rather
  than imported, an empty or literal-free artefact is refused, the writer is
  atomic and preserves the file mode, and a stale-binding failure is repairable
  while a bad signature or wrong anchor hard-fails. (`75881a55`, `628e16b5`,
  `75f2bbc4`)
- **`pip install .` no longer drops `AMA_INTEGRITY_REQUIRE_TRUST_ANCHOR=1`**
  from the signer, and Linux wheels are re-signed after `auditwheel repair`
  (`tools/resign_wheel.py`). (`17e5d79d`, `75f2bbc4`)
- **Owner-only storage is enforced on Windows** with a protected DACL
  (`ama_cryptography/_owner_only.py`) for the key store, key files, KDF
  metadata and nonce ledger; `chmod` had done nothing there. (`28b70b51`)

Input handling and fail-closed behaviour:

- **ML-KEM SampleNTT no longer truncates its XOF window** (FIPS 203
  Algorithm 7): it squeezed a fixed 672 octets and could leave coefficients
  uninitialised (probability about 1e-39 per matrix entry); output is
  byte-identical on every seed. (`75881a55`)
- **SHA-3 streaming misuse is refused**: a squeeze position from another
  family wrapped `available` and read past the Keccak state; a squeeze after
  `final` returned constants (row 11); `buffer_len` is checked per rate.
  (`75f2bbc4`, `611a41f5`, `94e4bd68`, `7766faaf`)
- **Heap overflow through `AmaContext`**: `sign`, `kem_encapsulate`,
  `kem_decapsulate` and `keypair_generate` trusted a caller-declared output
  length; capacity is now read from the buffer itself. `AmaContext.verify`
  returns `bool` (it returned the raw code, so `if ctx.verify(...)` accepted
  forgeries). (`9b860868`, `2dcc282`)
- **Health-tested randomness everywhere** (INVARIANT-41): the RFC 3161 replay
  nonce, session IDs and the responder handshake session ID (row 12) no longer
  use bare `secrets` draws; the continuous RNG test is locked; the Windows and
  POSIX RNG tails are written in full. (`fe8b8ab4`, `75f2bbc4`, `94e4bd68`,
  `7766faaf`)
- **RFC 3161 "disabled" mode failed open**: a token was downgradable to none;
  it now needs `allow_disabled=True` from the caller. An oversized `TSTInfo`
  nonce is refused as malformed. (`9b860868`, `17e5d79d`)
- **Selectors resolve exactly or refuse** (INVARIANT-35, row 3): an escalation
  from an unrankable algorithm is refused instead of resolving to the weakest
  rung; `jwk_thumbprint` accepts six named hashes only, none through OpenSSL
  (row 15, INVARIANT-1).
  (`fe8b8ab4`, `7766faaf`)
- **Dispatch and build hardening**: the dispatch table is sealed read-only
  after initialisation; the cache writer uses `O_EXCL|O_NOFOLLOW` and the
  reader refuses FIFOs; `AMA_DISPATCH_*` knobs are ignored under setuid;
  `-mavx2` no longer leaks into portable code (pre-AVX2 wheels could `SIGILL`;
  the default build now has zero AVX opcodes outside src/c/avx2 /
  src/c/avx512);
  ML-KEM's secret divisions use an exact reciprocal (KyberSlash); CET and
  BTI/PAC-RET are requested explicitly for the library and the Cython
  extensions. (`1f43143f`, `e8b9c8c1`, `7766faaf`, `c8081aff`)
- **Nonce ledger and key store**: nonce-tracker capacity is bounded by refusal
  (never eviction) and the ledger is created `0600` with `O_NOFOLLOW`;
  `delete_key` refuses a symlink or FIFO and overwrites in binary mode.
  (`1f43143f`, `0cf4fc0d`, `d6270f28`)
- **No OpenSSL in the published container images**, and
  `tools/check_vendor_isolation.py` now checks linkage, resident modules,
  container recipes and the CMake configuration. (`fe8b8ab4`, `94e4bd68`)
- **Not fixed by the 2026-09-16 remediation, stated so it is not assumed**:
  the release signing seed is in scope for unpinned PyPI build dependencies
  (A-7), and tag-signature preflight ordering (A-8) is a process item; both
  were left to the maintainer. FROST follows RFC 9591's structure but is **not** RFC 9591
  ciphersuite-interoperable, and the header says so. (`9b860868`)

### Fixed

Library and package:

- **Python/C boundary** (INVARIANT-5): one `_byte_view` rule for every borrowed
  buffer (1-D, byte items, contiguous, else `TypeError`); the ML-DSA hedged
  signer, both FROST rounds, PBKDF2 and SHA-384/512 accept `bytearray` and
  `memoryview`; `ama_hmac_sha256` with a NULL input is refused at the Python
  boundary. (`0cf4fc0d`, `9b860868`)
- **Legacy packages**: a package created without an ML-DSA backend failed its
  own verification, and the RFC 3161 token sat outside both authenticators;
  both transcripts are now built once, after every field is final.
  (`0cf4fc0d`)
- **Integrity signer identity**: one parser for the interpreter command line
  (`-c`, `-W`, `-X` and clustered flags no longer read as a signer launch);
  the Linux release re-sign chain recognises the signer via `sys.orig_argv`.
  (`0cf4fc0d`, `191befb`, `94e4bd68`)
- **ML-KEM**: `kyber_compress_d` applies its own `mod 2^d` and checks its
  width bound first; a rejected encapsulation key raises `ValueError`, not
  `KyberUnavailableError`. (`2ac912e9`, `75881a55`, `e9a52c91`)
- **ML-DSA's inverse NTT stays inside `int32`**: three call sites fed it an
  unreduced l-fold accumulator (worst case about 5x over `INT32_MAX`); outputs
  are byte-identical and `tests/c/test_dilithium_invntt_bound.c` pins the
  bound. (`c8081aff`)
- **A data race in the shipped library**: `nistp_use_mulx4()` cached CPUID in
  a plain `int` on attacker-input paths (INVARIANT-15); it is `_Atomic`, and
  `tests/c/test_concurrent_init.c` gives ThreadSanitizer real concurrency.
  (`f8873ae8`)
- **Undefined behaviour**: NEON Kyber/Dilithium `vmulq` signed overflow,
  Ed25519 unaligned key loads, a `(uint64_t)x << d` before its guard, `memcpy`
  from NULL in `ama_hkdf`, and Argon2id length truncation above `UINT32_MAX`.
  (`0bf9ce6`, `f8873ae8`, `75881a55`, `7766faaf`)
- **Portability**: MinGW-w64 builds (`_WIN32` rather than `_MSC_VER` guards,
  `InitOnceExecuteOnce` under INVARIANT-15), `fe51.h` on MSVC, MSVC ARM64
  linking, the Windows dispatch-seal alignment, the macOS universal2 CFI flags
  per slice, big-endian AArch64 refused at build time for the NEON tier, and
  `AMA_USE_NATIVE_PQC=OFF` test targets. (`628e16b5`, `1054c826`, `8157a86d`,
  `28b70b51`, `611a41f5`)
- **Monitoring and adaptive posture** (row 22): failed rotations back off and
  stop; escalation is no longer permanent (a bounded, decaying accumulator
  scores each alert once); the resonance detector no longer reports resonance
  for a constant series; the ratio matrix is capped; a clock regression cannot
  blind detection; `sigma_quadratic` enforcement uses the symmetric part.
  (`8157a86d`, `75881a55`, `fe8b8ab4`, `17e5d79d`, `2466fdf`)
- **Smaller fixes**: `create_crypto_package` rejects `num_derived_keys < 1`
  (row 16) and unknown `tsa_mode` values (row 17); `verify_crypto_package`
  records a present-but-unverifiable layer as `False`; `_self_test.last_failure()`
  is recorded when POST fails; `SecureKeyStorage` writes KDF metadata atomically;
  `python -m ama_cryptography` parses its arguments; `legacy_compat.secure_wipe()`
  delegates to the native barrier-backed `secure_memzero`; a skipped benchmark
  names the exception that skipped it (INVARIANT-3). (`7766faaf`, `2dcc282`,
  `e3453848`, `150a99c3`, `c126037c`, `078af92f`, `1bf806b6`)
- **Records Windows dropped**: 29 log literals a cp1252 file handler could not
  encode — including the key-rotation audit records — are rewritten, and
  INVARIANT-43 keeps them so. (`1025a187`)
- **Builds**: the Ed25519 MULX translation unit compiles under Clang with a
  frame pointer (it failed in the Valgrind lane); the fat-LTO static archive
  carries no bytecode; `.clang-format` is readable by clang-format before 20.
  (`0207b2ec`, `611a41f5`, `94e4bd68`)

Documentation corrected against the code (INVARIANT-53):

- **Examples that ran wrongly**: the wiki's C example minted Ed25519 private
  keys from uninitialised stack memory; 33 of 38 C prototypes in
  `wiki/C-API-Reference.md` were wrong (among them a 32-byte Ed25519 secret key
  and AEAD argument order); the Flask and FastAPI examples called a parameter
  that does not exist. All are regenerated from the headers and executed in CI.
  (`bb600dbe`, `f8873ae8`, `fe8b8ab4`)
- **Claims corrected**: posture weights and thresholds, the hybrid combiner's
  absent fallback, HSS/LMS verification being exported, `ama_randombytes` not
  being exported, the compiler floor, the ML-DSA signing cost, `HDKeyDerivation`
  not being BIP32-compliant (KM-HD-001: its master key uses a different HMAC key,
...
- **Every build signs its own integrity artefact** and binds the binding
  extensions it ships (rows 19, 20); `_integrity_signature.py` is a build
  output, untracked and excluded from the sdist, and `_integrity_digest.txt`
  stays tracked as the source-drift check. An unbuilt clone's import error
  names `pip install -e .` / `python setup.py build_ext --inplace`.
  (`8157a86d`, `514c0942`)
- **The 3R timing-anomaly detector is rebuilt against its measured evidence**
  (row 8): a robust score against a calibrated per-operation alarm budget,
  edge-triggered sign-CUSUM shift events after a 200-sample reference lock,
  `TimingAnomaly.kind`, and `critical` reachable only once calibrated;
  `benchmarks/detector_baseline_eval.py` runs as a CI gate. (`94e4bd68`,
  `8d72b8ca`)
- **`CryptoPostureController` ranks per algorithm family** and refuses an
  algorithm it cannot rank (row 3); `confirm_action()` no longer reports a
  suppressed switch or rotation as executed. (`7766faaf`, `611a41f5`)
- **Dispatch**: `AMA_DISPATCH_ONLY` names every installable kernel (new
  `sha3-avx2x4`, `aes-gcm-aesni`, `aes-gcm-vaes`, `dilithium-ntt-sve2`,
  `kyber-ntt-neon`, `dilithium-ntt-neon`, `argon2-g-neon`), pins survive
  auto-tune demotion, AES-NI no longer depends on AVX2, the single-state AVX2
  Keccak and the scalar-code `kyber_pointwise` kernels are no longer installed,
  and the dispatch report shows the wired AES-GCM backend. (`5d9015d8`,
  `c8081aff`, `8ddf6062`, `fe8b8ab4`, `e8b9c8c1`)
- **Hashing moves off `hashlib`** (INVARIANT-1): production hashing and PBKDF2
  run on AMA's own kernels, including the BIP39 seed and the FIPS self-test
  paths (byte-identical); `hashlib` is confined to the pre-execution trust
  bootstrap and pinned file by file. (`d14ad84`, `4eb9107`, `2371184`,
  `c4c83479`)
- **Ed25519 is one in-house implementation**: comb and safegcd rewrites,
  half-size verification, and a radix-2^64 MULX instantiation selectable at run
  time; the vendored backend's verdicts are frozen in
  `tests/oracle/ed25519_frozen_oracle.txt` and replayed on every build.
  (`1054c826`)
- **Integrity checks**: POST treats `.pyc` staleness as CPython does (row 10);
  a tree with uncovered bindings reports `signed-bindings-unverified` (row 9);
  `tools/verify_install_oob.py` requires an out-of-tree `--expected-pubkey`.
  (`75f2bbc4`, `fe8b8ab4`)
- **Toolchain floors** (Dependabot #401 and #402 folded in): `setuptools`
  84.0.0, `wheel` 0.48.0 (GHSA-vgq5-9859-3mmw), `cmake` 4.4.3, `Cython` 3.3.0,
  `build` 1.6.1, `ruff` 0.16.8, `mypy` 2.3.1, and the SHA-pinned actions at their
  current tags; #392 is obsolete. (`a0826d12`, `0695024e`, `ffb51ba2`,
  `0a1be889`)
- **Published benchmark figures**: the 4.x canonical-host tables are retired;
  `README.md` publishes CI four-run medians only, each with runs, jobs and
  build flags, pinned by `tools/check_published_benchmarks.py` against
  `benchmarks/published-benchmarks.json` (which replaces
  `check_canonical_benchmarks.py`); AVX-512-class figures wait for such
  hardware at the release commit. About 180 further hand-written figures in 17
  documents without host, flags or run are removed or replaced, the gate now
  pins README plus two wiki pages (215 figures) keyed by page, row and
  position, and the stale 3.4.0 `benchmarks/dashboard.html` is deleted.
  (`6342a215`, `a62bb446`)

### Added

- **Public C API**: `ama_ed25519_expand_secret_key` /
  `ama_ed25519_sign_expanded` (INVARIANT-51 checked once at load, signature
  bytes identical), `ama_frost_verify_share`, `ama_slhdsa_sign_addrnd`,
  `ama_ml_dsa_sign_hedged` (FIPS 204 Algorithm 2), `ama_secp256k1_ecdsa_sign_raw`
  (fixed 64-octet `r || s`), `ama_sha512`, `ama_sha384`, `ama_sha3_384`,
  `ama_pbkdf2_hmac_sha256` / `_sha512`, `ama_secure_stack_wipe`,
  `ama_dispatch_table_is_sealed`, `ama_ed25519_set_mulx_override` /
  `ama_ed25519_active_backend`, and a declaration for `ama_sha256`.
  (`24341f89`, `9b860868`, `e8b9c8c1`, `4d0838ca`, `d14ad84`, `1f43143f`,
  `1054c826`, `549980b5`)
- **Python**: `pqc_backends.Ed25519SigningKey` and
  `crypto_api.Ed25519Provider.signing_key()`; `native_sha512/384`,
  `native_sha3_384`, `native_pbkdf2_hmac_sha256/512`,
  `native_ml_dsa_sign_hedged`; `generate_slhdsa_keypair_from_seed` exported;
  `module_attestation()` and `AMA_POST_DIAGNOSTIC_IMPORT=1` for triage.
  (`24341f89`, `d14ad84`, `35891ca4`, `7766faaf`)
- **Invariants 39–53** register the controls above; see `INVARIANTS.md`.
- **Release trust store**: `.github/allowed_signers` publishes the release
  signing key, and `tests/test_release_tag_trust_store.py` verifies the v4.0.0
  tag object against it; consumers run `git -c
  gpg.ssh.allowedSignersFile=.github/allowed_signers verify-tag <tag>`.
  (`a2e343bd`)
- **Tools**: `tools/verify_install_oob.py` (a stdlib-only out-of-band verifier),
  `tools/resign_wheel.py`, `tools/acvp_vector_pin.py`,
  `tools/measure_branch_coverage.py`, and `fuzz/fuzz_nistp.c`.
  (`75f2bbc4`, `5d9015d8`, `839b66b4`, `b87d0256`)

### Removed

- **The vendored ed25519-donna backend**, removed in the twenty-first
  maintenance pass (2026-09-06) with its shim, `AMA_ED25519_ASSEMBLY`, the
  `AMA_ED25519_VERIFY_SHAMIR` / `_WINDOW` cache variables and the differential
  job; `src/c/vendor/` must not exist (INVARIANT-1). Its verdicts are frozen in
  the replayed oracle (see **Changed**). (`1054c826`)
- **Dead or wrong kernels**: the AVX2 Poly1305 helpers, three ML-KEM pointwise
  kernels, the AVX2 single-state Keccak, a NEON WOTS helper,
  `barrett_reduce_dil_neon`, `ama_ed25519_neon.c`, the `ama_sha3_256_*`
  wrappers behind the removed dispatch slot, and several uncalled C and Python
  symbols. (`9b860868`, `150a99c3`, `c8081aff`, `658f2fe7`, `0ef61ab`)
- **Removed from the public surface**: see *Wire-format and ABI changes*.
- **`AMA_REQUIRE_CONSTANT_TIME`** enforcement (the variable is deprecated and
  has no effect); the `scipy`, `pytest-xdist`, `pytest-timeout` and
  `pytest-benchmark` dependencies; the audit workspaces `docs/audit/` and
  `verification/v5-audit/`, which remain in commit history. (`658f2fe7`,
  `80c561ea`, `d7545bcf`)

### Performance

- **Ed25519, in-house against the removed vendored backend**: callgrind counts
  of 132,080 / 147,952 / 400,513 instructions for keygen / sign / verify against
  154,482 / 169,816 / 526,854 (x86-64, GCC 13 `-O3`, 2026-09-06). (`1054c826`)
- **Costs the security fixes add, measured**: INVARIANT-51 roughly doubles
  Ed25519 signing curve work, and `ama_ed25519_sign_expanded` takes it back —
  24,780 ns per-call against 13,613 ns expanded (2026-09-22,
  `benchmark_c_raw` median of 1,000, Intel Xeon @2.80GHz container, `taskset -c
  0`); INVARIANT-41's keygen cost on the hash-based parameter sets is glance
  row 4's. (`24341f89`, `7766faaf`)
- **SLH-DSA signing without a heap copy**: absorbing the §10.2 prefix into the
  streaming hashes, instead of materialising `M'`, made large-message signing
  faster than the §9 path it replaced and removed the last allocation from
  `src/c/ama_slhdsa.c`; the measurement is in `9b860868`.
- **AES-256-GCM without AVX2**: a build with `-DAMA_ENABLE_AVX2=OFF` had
  silently fallen back from AES-NI to the bitsliced path; hardware AES-GCM no
  longer depends on AVX2, and `fe8b8ab4` records the before/after measurement.
- **Python one-shot AEAD wrappers** give back the throughput the
  buffer-borrow hardening took, through a hand-written borrow with an
  all-`bytes` fast path; hybrid signing memoises its Ed25519 expansion
  (row 5). The A/B measurement is in `7766faaf`.
- **Benchmark floors** are four-run CI medians with derived tolerances, and the
  harness measures full-window batches over input pools; figures and runs are in
  `benchmarks/baseline.json` and `docs/BENCHMARK_HISTORY.md`. (`7766faaf`,
  `4a454089`, `219d3fa5`)

### Tooling & CI

- **Gates that could not fail were made able to**: CodeQL results are enforced
  from the SARIF (security-severity 7.0 and above); aggregating gates must
  evaluate every need; path-filtered workflows carry no-op twins so they can be
  required (INVARIANT-31); MemorySanitizer and ThreadSanitizer run on every
  trigger, the MemorySanitizer lanes with SIMD built; the reproducible-build, fuzz-dictionary, wiki-sync
  and Valgrind steps check what they report. (`e8b9c8c1`, `150a99c3`,
  `28b70b51`, `5d9015d8`, `7601f68a`)
- **Constant-time instruments**: the callgrind gate compares instructions, data
  references and D1/LLd misses with pinned cache geometry, holds every target
  to a zero cross-class delta, refuses an unoptimised build, and runs under
  `AMA_DISPATCH_NO_AUTOTUNE=1`; dudect uses a calibrated cropped statistic
  (threshold 5.0), per-class staging and a 2 ns effect-size floor with an
  explicit `SUB-FLOOR` verdict. (`e52281c3`, `4d0838ca`, `2e7f1ca3`,
  `1f8a23e1`, `e46906c7`)
- **Pins and provenance**: every third-party action is SHA-pinned and resolves
  to a release tag (INVARIANT-4, INVARIANT-24); the ACVP projections are pinned
  by SHA-256 (INVARIANT-44); vendored vectors are pinned in
  `tests/kat/PROVENANCE.json`; `tests/kat/fips203/ml_kem_1024.kat` is attributed
  to its real source and genuine ACVP groups are vendored. (`fe8b8ab4`,
  `e8b9c8c1`, `5d9015d8`, `4d0838ca`)
- **Coverage that runs**: a per-slot KAT sweep runs the published vectors under
  every SIMD pin (INVARIANT-45); fuzz corpora persist and deepen, and the
  OSS-Fuzz build is exercised (INVARIANT-46); a lane that provisions a resource
  fails the skip of it (INVARIANT-47); SoftHSM2 runs a real token on Linux and
  Windows. (`5d9015d8`, `150a99c3`, `2ac912e9`, `75f2bbc4`)
- **Static analysis at source**: no suppression in `src/c/` or `include/`
  (INVARIANT-13; the one exception once recorded was retracted), `cppcheck`
  suppressions removed by fixing the code, `-Wsign-conversion` on in every
  strict lane, and the strict-warnings gate built at Release+LTO and for
  AArch64 NEON+SVE2. (`4a454089`, `8ddf6062`, `94e4bd68`)
- **Documentation gates** (INVARIANT-53): `check_doc_examples.py`,
  `check_crypto_construction_docs.py`, `check_public_api_docs.py`,
  `check_benchmark_claims.py` and `check_published_benchmarks.py`, plus the
  algorithm-registry, keygen-PCT, type-check-scope, documented-counts and
  documented-source-path gates. (`26043536`, `fe8b8ab4`, `f8873ae8`,
  `6342a215`)
- **Gate bypasses closed**: file enumeration goes through `tools/_repo.py` with
  `-z` (a non-ASCII filename skipped the secret scan); benchmark claims are
  attributed to one benchmark and one architecture; every exported `ama_*`
  symbol must be declared in an installed header. (`549980b5`)
- **CI economy**: each pytest lane runs once per head (the ubuntu and windows
  legs of `ci-build-test.yml::python-package` duplicated `ci.yml::test`), and
  push triggers are scoped to `main`/`develop`. (`c64e6ccf`)
- **Release gates**: a `v*` tag fails preflight while a release-state document
  still says the version is unreleased (`tools/check_release_state.py`); a
  release needs a reproducible wheel; the agent-binding timing lane measures the
  check rather than the GIL. (`41a3586`, `87ba594a`, `d6270f28`)
- **Tests that pin guards no test reached**: the Ed25519 half-size reduction's
  degenerate pair, §5.1.3 on the non-verify decoders, the P-curve NULL-argument
  guards, the Ed25519 signing scrubs, and fuzz harnesses that assert contracts
  rather than "no crash". (`4329c7d3`, `839b66b4`, `abde8640`, `c126037c`,
  `e8b9c8c1`)
- **Derived documents regenerate**: `tools/update_docs.py --counts` and
  `--loc` rewrite the counts and line totals the gates check, so a stale figure
  has a one-command fix. (`2739795f`, `8ddf6062`, `75f2bbc4`)
- **Repo-wide gates check the tracked tree**: `check_docker_pins`,
  `check_crypto_construction_docs`, `check_version_consistency` and
  `check_vendor_isolation --build-config` walked the filesystem, so an
  untracked nested checkout failed them; they enumerate through
  `tools/_repo.py` and fail closed when git cannot. (`8e81231a`)
- **Artefact-tracking tests fixed after CI**: the sdist test now adds files
  through `FileList.append` (Windows could never match it before), and the
  git-probe helper binds its result on every path CodeQL follows. (`55456b1e`)
