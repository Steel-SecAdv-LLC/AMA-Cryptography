...
> **Status: Aspirational.** This invariant documents the desired end state but
> is **not enforced** in the current codebase. An earlier implementation attempt
> using `bytearray` secret keys with `wipe()`/`__del__` destructors was reverted
> due to cascading incompatibilities: `ctypes.c_char_p` rejects `bytearray`,
> transient immutable `bytes` copies are created at conversion boundaries,
> `except Exception: pass` in destructors violates INVARIANT-3, and shared-reference
> wipe caused use-after-wipe bugs in providers. Do not re-introduce without a
> design that resolves all of these issues.

PQC key-pair dataclasses (`DilithiumKeyPair`, `KyberKeyPair`, `SphincsKeyPair`)
**should** eventually store secret key material in mutable buffers so that it
can be securely zeroed via `secure_memzero`. Until the design issues above are
resolved, secret keys remain as immutable `bytes`.
