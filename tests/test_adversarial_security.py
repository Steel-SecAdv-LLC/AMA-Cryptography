...
skip_no_kyber = pytest.mark.skipif(
    not KYBER_AVAILABLE, reason="Kyber not available"
)
...
skip_no_aes_gcm = pytest.mark.skipif(
    not _AES_GCM_NATIVE_AVAILABLE, reason="AES-GCM not available"
)
...
def _aes_gcm_encrypt(key, nonce, pt, aad=b""):
    ...
    rc = _native_lib.ama_aes256_gcm_encrypt(
        key, nonce, pt, ctypes.c_size_t(len(pt)),
        aad, ctypes.c_size_t(len(aad)), ct, tag
    )
    assert rc == 0
...
def _aes_gcm_decrypt(key, nonce, ct, tag, aad=b""):
    ...
    rc = _native_lib.ama_aes256_gcm_decrypt(
        key, nonce, ct, ctypes.c_size_t(len(ct)),
        aad, ctypes.c_size_t(len(aad)), tag, pt
    )
    return rc, bytes(pt)
...
class TestFaultInjection:
    ...
    def test_dilithium_sig_bit_flip(self):
        ...
        for pos in [0, len(sig) // 2, len(sig) - 1]:
            ...
            assert not _dilithium_verify(pk, msg, bad_sig), \
    ...
    def test_dilithium_pk_bit_flip(self):
        ...
        for pos in [0, DIL_PK // 2, DIL_PK - 1]:
            ...
            assert not _dilithium_verify(bad_pk, msg, sig), \
    ...
    def test_kyber_ct_bit_flip(self):
        ...
        for pos in [0, KYBER_CT // 2, KYBER_CT - 1]:
            ...
            assert ss_dec != ss_enc, \
                f"Shared secret should differ with corrupted ct at byte {pos}"
...
class TestSecurityEdgeCases:
    ...
    def test_aes_gcm_truncated_ciphertext(self):
        ...
        for trunc_len in [0, 1, len(ct) // 2, len(ct) - 1]:
            ...
            rc = _native_lib.ama_aes256_gcm_decrypt(
                key, nonce, truncated, ctypes.c_size_t(trunc_len),
                b"", ctypes.c_size_t(0), tag, pt_buf
            )
