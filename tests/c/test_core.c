...
int main(void) {
    ...
    TEST_ASSERT(strcmp(version_str, AMA_CRYPTO_VERSION_STRING) == 0, "version_string: should be '" AMA_CRYPTO_VERSION_STRING "'");

    /* Test 2: Version number */
    ava_version_number(&major, &minor, &patch);
    TEST_ASSERT(major == AMA_CRYPTO_VERSION_MAJOR && minor == AMA_CRYPTO_VERSION_MINOR && patch == AMA_CRYPTO_VERSION_PATCH, "version_number: should be " AMA_CRYPTO_VERSION_STRING);
