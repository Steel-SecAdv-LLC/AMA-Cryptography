...
class TestNonceTracker:
    ...
    def test_different_keys_same_nonce_ok(self, tmp_path: Path) -> None:
        tracker = NonceTracker(persist_path=str(tmp_path / "nonces.dat"))
        tracker.check_and_record(b"key1", b"\x00" * 12)
...
class TestRuntimeIntegrity:
    ...
    def test_monitor_verify_runtime_integrity(self, tmp_path: Path) -> None:
...
class TestMonitorIntegration:
    ...
    def test_nonce_tracking_via_monitor(self, tmp_path: Path) -> None:
        monitor = AmaCryptographyMonitor(
            enabled=True,
            nonce_persist_path=str(tmp_path / "nonces.dat"),
        )
    ...
    def test_key_lifecycle_via_monitor(self, tmp_path: Path) -> None:
        monitor = AmaCryptographyMonitor(
            enabled=True,
            nonce_persist_path=str(tmp_path / "nonces.dat"),
        )
    ...
    def test_disabled_monitor_skips_all(self, tmp_path: Path) -> None:
        monitor = AmaCryptographyMonitor(
            enabled=False,
            nonce_persist_path=str(tmp_path / "nonces.dat"),
        )
