...
def _setup_argon2_ctypes(lib: ctypes.CDLL) -> bool:
    ...
    The two legacy-migration entry points (``ama_argon2id_legacy``,
    ``ama_argon2id_legacy_verify``) are bound opportunistically: older
    shared libraries built before 2.1.6 do not export them, and their
    absence must not block the spec-compliant ``ama_argon2id`` wrapper
    from loading.  Callers that need the migration shim get a clear
    RuntimeError from :func:`native_argon2id_legacy_verify` at call
    time instead of an import-time failure.
    """
    try:
...
def native_argon2id_legacy_verify(
    ...
    """Constant-time verify a pre-AMA-2.1.5 Argon2id tag.

    AMA ≤ 2.1.5 shipped a ``blake2b_long`` loop-termination bug (see
    CHANGELOG.md [Unreleased] § BREAKING). Stored hashes derived by
    those versions sit in a non-spec bit-space and will not verify
    against the post-fix :func:`native_argon2id`. This helper
    reproduces the legacy derivation and compares against
    ``expected_tag`` with ``ama_consttime_memcmp`` so a deployment can
    run the "verify-with-legacy, re-derive-with-fixed, overwrite"
    migration recommended in the changelog without forking the old
    code.

    Args:
        password:     Password bytes.
        salt:         Salt bytes (same ≥ 8-byte minimum as
    ...
    if not hasattr(_native_lib, "ama_argon2id_legacy_verify"):
        raise RuntimeError(
            "ama_argon2id_legacy_verify() is not exported by the loaded "
            "native library — rebuild against AMA Cryptography >= 2.1.6 "
            "to enable the pre-2.1.5 migration shim."
        )
    ...
    if parallelism < 1 or parallelism > _UINT32_MAX:
        raise ValueError(
            f"Argon2id parallelism must be in [1, {_UINT32_MAX}], got {parallelism}"
        )
    if m_cost < 8 * parallelism or m_cost > _UINT32_MAX:
        raise ValueError(
            f"Argon2id m_cost must be in [{8 * parallelism}, {_UINT32_MAX}] KiB, "
