...
try:
    ...
    _PyKCS11_module = None  # PyKCS11 unavailable
    HSM_AVAILABLE = False
...
class HSMKeyStorage:
    ...
    def generate_ec_keypair(
        ...
        curve_oid_map = {
            "P-256": bytes([0x06, 0x08, 0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x03, 0x01, 0x07]),
            "P-384": bytes([0x06, 0x05, 0x2B, 0x81, 0x04, 0x00, 0x22]),
            "P-521": bytes([0x06, 0x05, 0x2B, 0x81, 0x04, 0x00, 0x23]),
        }
    ...
    def sign(self, key_handle: bytes, data: bytes) -> bytes:
        """Sign *data* using the HSM-resident private key (ECDSA).
