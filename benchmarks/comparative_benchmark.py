...
class ComparativeBenchmark:
    ...
    def calculate_comparative_metrics(self) -> Dict:
        ...
        for operation, results in by_operation.items():
            ...
            ava_result = next((r for r in results if r.implementation == "AMA Cryptography"), None)
            if not ava_result:
                continue

            print(f"\n{operation}:")
            print(
                f"  AMA Cryptography: {ava_result.mean_time_ms:.4f}ms ({ava_result.ops_per_sec:.2f} ops/sec)"
            )
