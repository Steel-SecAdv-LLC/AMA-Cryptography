...
THRESHOLDS = {
    ...
    "sha3-256": 0,
    "ed25519-sign": 0,
    # The same driver with the SSE2 Niels-select fold forced through
    # ama_ed25519_set_avx2_fold_override(0).  Every CI runner has AVX2, so
...
_REMEDY = {
    ...
    "ed25519-sign-sse2fold": (
        "Same subject as ed25519-sign with the SSE2 Niels-select fold forced\n"
        "(ama_ed25519_set_avx2_fold_override(0)).  A delta here and not on\n"
        "ed25519-sign localises the defect to the SSE2 fold in\n"
...
int main(int argc, char **argv) {
    ...
    /* SSE2 fold forced before any Ed25519 work (measurement knob, see the
     * header): this target exists to measure the fold AVX2 hosts skip. */
    ama_ed25519_set_avx2_fold_override(0);
...
# ===========================================================================
# Taint mode (``--taint``): secret-dependent branches and addresses, not counts
# ===========================================================================
#
# Callgrind's four metrics answer one question: does the retired-instruction
# stream, the number of data references and the number of simulated cache
# misses depend on the secret?  A secret-INDEXED load into a table that fits
# in the simulated L1 answers "no" on all four -- one load either way, no
# miss either way -- and so does a secret-selected indirect call between two
# equally long functions.  Both were planted into ML-KEM decapsulation in a
# scratch build and both passed the ``kyber-decaps`` gate above with every
# delta at zero.  The statement this file used to make, that "the
# data-reference and miss figures rule out a secret-dependent access", was
# therefore too strong, and is withdrawn: they rule out an access pattern that
# changes the COUNT of references or misses, which a cache-resident lookup
# does not.
#
# This mode asks the question directly, the way ctgrind (Adam Langley, 2010)
# did: mark the secret bytes UNDEFINED with a Memcheck client request and run
# the primitive.  Memcheck tracks definedness bit-for-bit through every
# arithmetic operation and reports the moment an undefined value decides a
# conditional jump, a conditional move, or a memory ADDRESS -- which is
# exactly the set of things a constant-time implementation must never let a
# secret decide.  No statistics, no threshold, no cache model: a single
# secret-indexed byte load is reported with a stack trace.  The same five
# plants that passed the instruction-count gate are all reported here.
#
# What it cannot see: data-operand-dependent LATENCY of an instruction whose
# operands are secret (a multiplier that is faster on small operands, a
# divider).  That is the KyberSlash class, covered by
# tools/check_secret_division.py reading the shipped object, and the
# deployment-mode question (Intel DOITM / ARM PSTATE.DIT) that no
# instruction-level tool answers.
#
# A driver in this mode taints ONLY the secret (a key, a seed, the parts of a
# decapsulation key that FIPS 203 calls secret) and marks the primitive's
# return code defined before branching on it, so the only reports are the
# library's own.  Memcheck flags a conditional MOVE on a secret as well as a
# conditional jump; a cmov is constant-time on x86-64, so a report of that
# shape is a finding about the code's shape (a secret decided a select) and
# is still worth reading, but it is reported with the instruction that raised
# it so the two can be told apart.
...
#include "ama_cryptography.h"
#define TAINT(ptr, len) VALGRIND_MAKE_MEM_UNDEFINED((ptr), (len))
#define UNTAINT(ptr, len) VALGRIND_MAKE_MEM_DEFINED((ptr), (len))
"""

_TAINT_DRIVERS: dict[str, str] = {
    "kyber-decaps": _TAINT_PRELUDE
    + r"""
/* dk = s || ek || H(ek) || z.  Secret: s (the 1536-byte NTT-domain secret
 * vector) and z (the implicit-rejection seed).  Both a valid and a corrupted
 * ciphertext are decapsulated, so the FO compare-and-select runs both ways
 * under taint. */
int main(void) {
    static uint8_t pk[AMA_KYBER_1024_PUBLIC_KEY_BYTES];
    static uint8_t sk[AMA_KYBER_1024_SECRET_KEY_BYTES];
    ...
    uint8_t d[32], z[32];
    size_t ct_len = sizeof ct;
    for (unsigned i = 0; i < 32; i++) { d[i] = (uint8_t)(0x11u + i); z[i] = (uint8_t)(0x77u + i); }
    if (ama_kyber_keypair_from_seed(d, z, pk, sk) != AMA_SUCCESS) return 1;
    if (ama_kyber_encapsulate(pk, sizeof pk, ct, &ct_len, ss, sizeof ss) != AMA_SUCCESS) return 1;
    memcpy(ct_bad, ct, sizeof ct_bad);
    ct_bad[0] ^= 0xFFu;
...
""",
    "ed25519-sign": _TAINT_PRELUDE
    + r"""
/* Secret: the 32-byte seed the expanded key is derived from; everything
 * derived from it (the clamped scalar, the prefix, the nonce, the public key
 * as it flows through the signature hash) inherits the taint. */
int main(void) {
    ...
    static volatile uint8_t sink;
    for (int i = 0; i < 2; i++) {
        ama_error_t rc = ama_ed25519_sign(signature, message, sizeof message, secret_key);
        UNTAINT(&rc, sizeof rc);
        if (rc != AMA_SUCCESS) return 1;
        sink = (uint8_t)(sink ^ signature[0]);
    }
...
""",
    "x25519": _TAINT_PRELUDE
    + r"""
/* Secret: the scalar.  The ladder's conditional swap is driven by one secret
...
int main(void) {
    ...
    static volatile uint8_t sink;
    for (int i = 0; i < 2; i++) {
        ama_error_t rc = ama_x25519_key_exchange(out, sk, peer);
        UNTAINT(&rc, sizeof rc);
        if (rc != AMA_SUCCESS) return 1;
        sink = (uint8_t)(sink ^ out[0]);
    }
...
""",
    "x25519-batch": _TAINT_PRELUDE
    + r"""
/* Secret: all four lane scalars.  The aggregated low-order rejection must be
 * a mask over the lanes, never a branch on any of them. */
int main(void) {
    uint8_t sk[4][32], peer[4][32], out[4][32];
    for (unsigned lane = 0; lane < 4; lane++)
        for (unsigned i = 0; i < 32; i++) {
            sk[lane][i] = (uint8_t)(0x41u * 31u + i * 167u + i * i * 13u + lane * 97u);
            peer[lane][i] = (uint8_t)(0x09u + i + lane * 5u);
        }
    TAINT(sk, sizeof sk);
    ama_error_t rc = ama_x25519_scalarmult_batch(out, (const uint8_t (*)[32])sk,
                                                 (const uint8_t (*)[32])peer, 4);
    UNTAINT(&rc, sizeof rc);
    if (rc != AMA_SUCCESS) return 1;
    static volatile uint8_t sink;
    for (int lane = 0; lane < 4; lane++) sink = (uint8_t)(sink ^ out[lane][0]);
    return 0;
}
""",
    "consttime": _TAINT_PRELUDE
    + r"""
/* Secret: both operands (a MAC tag and its recomputation).  An early exit on
 * the first differing byte is a branch on the difference and is reported. */
int main(void) {
    enum { N = 4096 };
    static uint8_t a[N], b[N];
    memset(a, 0x5a, sizeof a);
    ...
    TAINT(a, sizeof a);
    TAINT(b, sizeof b);
    int r = ama_consttime_memcmp(a, b, sizeof a);
    UNTAINT(&r, sizeof r);
    return r == 1 ? 0 : 1;
}
""",
    "ghash": _TAINT_PRELUDE
    + r"""
/* Secret: the AES-256 key, and through it the GHASH subkey H.  The scalar
 * (bitsliced AES + masked GHASH) path is forced, as in the count mode. */
void ama_test_force_aes_gcm_scalar(void);
int main(void) {
    uint8_t key[32], nonce[12], pt[512], aad[64], ct[512], tag[16];
    ...
    ama_error_t rc = ama_aes256_gcm_encrypt(key, nonce, pt, sizeof pt, aad, sizeof aad, ct, tag);
    UNTAINT(&rc, sizeof rc);
    if (rc != AMA_SUCCESS) return 1;
    static volatile uint8_t sink;
    sink = (uint8_t)(sink ^ tag[0] ^ ct[0]);
    return 0;
}
""",
    "ascon-encrypt": _TAINT_PRELUDE
    + r"""
/* Secret: the Ascon-AEAD128 key. */
int main(void) {
    ...
    memset(pt, 0xA5, sizeof pt);
    TAINT(key, sizeof key);
    ama_error_t rc = ama_ascon_aead128_encrypt(key, nonce, pt, sizeof pt, NULL, 0, ct, tag);
    UNTAINT(&rc, sizeof rc);
    if (rc != AMA_SUCCESS) return 1;
    static volatile uint8_t sink;
    sink = (uint8_t)(sink ^ ct[0] ^ tag[0]);
...
""",
    "agent-binding": _TAINT_PRELUDE
    + r"""
/* Secret: the authority key, and the authorization the binding stores.  A
 * good and a tampered binding are both checked; the verdict must be
 * computed with masks, never selected by a branch on the comparison. */
int main(void) {
    ama_agent_binding_t good, bad;
    uint8_t instance_id[AMA_AGENT_INSTANCE_ID_BYTES];
    uint8_t profile[AMA_ETHICAL_PROFILE_BYTES];
    ...
    TAINT(good.authorization, sizeof good.authorization);
    TAINT(bad.authorization, sizeof bad.authorization);
    static volatile unsigned sink;
    ama_error_t rc_good = ama_agent_binding_check(&good, authority_key, sizeof authority_key);
    ama_error_t rc_bad = ama_agent_binding_check(&bad, authority_key, sizeof authority_key);
    UNTAINT(&rc_good, sizeof rc_good);
    UNTAINT(&rc_bad, sizeof rc_bad);
    sink ^= (unsigned)rc_good ^ (unsigned)rc_bad;
    return (rc_good == AMA_SUCCESS && rc_bad != AMA_SUCCESS) ? 0 : 1;
}
""",
    "aead-verify": _TAINT_PRELUDE
    + r"""
/* Secret: the key.  The accept and the reject path are both driven; a branch
 * on the tag comparison (whose operands derive from the key) is reported. */
void ama_test_force_aes_gcm_scalar(void);
int main(void) {
    uint8_t key[32], nonce[12], aad[32], tag_good[16], tag_bad[16];
...
""",
    "secp256k1-scalarmult": _TAINT_PRELUDE
    + r"""
/* Secret: the scalar k. */
int main(void) {
    static const uint8_t Gx[32] = {
    ...
    static const uint8_t Gy[32] = {
        0x48,0x3A,0xDA,0x77,0x26,0xA3,0xC4,0x65,0x5D,0xA4,0xFB,0xFC,0x0E,0x11,0x08,0xA8,
        0xFD,0x17,0xB4,0x48,0xA6,0x85,0x54,0x19,0x9C,0x47,0xD0,0x8F,0xFB,0x10,0xD4,0xB8 };
    uint8_t k[32], out_x[32], out_y[32];
    for (unsigned i = 0; i < sizeof k; i++) k[i] = (uint8_t)(0x41u * 31u + i * 167u + i * i * 13u);
    k[0] &= 0x7Fu;
    TAINT(k, sizeof k);
    ama_error_t rc = ama_secp256k1_point_mul(k, Gx, Gy, out_x, out_y);
    UNTAINT(&rc, sizeof rc);
    if (rc != AMA_SUCCESS) return 1;
    static volatile uint8_t sink;
    sink = (uint8_t)(sink ^ out_x[0] ^ out_y[0]);
    return 0;
}
""",
    "ecdsa": _TAINT_PRELUDE
    + r"""
/* Secret: the secp256k1 private key.  The RFC 6979 nonce, the ladder, the
 * scalar arithmetic (Montgomery extra-reduction, carry folds, low-s
 * normalisation) and the r == 0 / s == 0 retry decision all derive from it.
 * The signature itself is public output, so the driver untaints only what
 * the API hands back. */
int main(void) {
    uint8_t sk[32], pk[33], sig[AMA_SECP256K1_ECDSA_RAW_SIG_LEN], msg[32];
    for (unsigned i = 0; i < sizeof sk; i++) sk[i] = (uint8_t)(0x41u * 31u + i * 167u + i * i * 13u);
    ...
    static volatile uint8_t sink;
    {
        ama_error_t rc = ama_secp256k1_pubkey_from_privkey(sk, pk);
        UNTAINT(&rc, sizeof rc);
        if (rc != AMA_SUCCESS) return 1;
    }
    for (int i = 0; i < 2; i++) {
        ama_error_t rc = ama_secp256k1_ecdsa_sign_raw(sig, msg, sk);
        UNTAINT(&rc, sizeof rc);
        if (rc != AMA_SUCCESS) return 1;
...
int main(void) {
    ...
    static volatile uint8_t sink;
    {
        ama_error_t rc = ama_nistp_pubkey_from_privkey(AMA_NIST_CURVE_P256, sk, pk);
        UNTAINT(&rc, sizeof rc);
        if (rc != AMA_SUCCESS) return 1;
    }
    for (int i = 0; i < 2; i++) {
...
_TAINT_REPORT_RE = re.compile(
    r"^==\d+== (Conditional jump or move depends on uninitialised value|"
    r"Use of uninitialised value|Syscall param .* uninitialised|"
    r"Invalid (read|write)|Jump to the invalid address)"
)
...
def _run_taint(driver: Path, workdir: Path) -> Optional[tuple[int, str]]:
    ...
    proc = subprocess.run(
        [
            ...
            "--error-exitcode=99",
            "--num-callers=12",
            f"--log-file={workdir / 'memcheck.log'}",
            str(driver),
        ],
        capture_output=True,
        text=True,
    ...
    log = (
        (workdir / "memcheck.log").read_text(encoding="utf-8", errors="replace")
        if (workdir / "memcheck.log").is_file()
        else ""
    )
    if proc.returncode not in (0, 99):
        print(
            f"  driver exited {proc.returncode} — the measured workload did not "
            "run to completion.\n" + log[-2000:],
            file=sys.stderr,
        )
        return None
    reports = sum(1 for line in log.splitlines() if _TAINT_REPORT_RE.match(line))
    if proc.returncode == 99 and reports == 0:
        # Memcheck said "errors" but none of the shapes above matched: report
        # the log rather than a false clean.
        reports = 1
    return reports, log
...
def _cmake_build_type(cache: Path) -> Optional[str]:
    """``CMAKE_BUILD_TYPE`` recorded in a CMakeCache.txt, or None."""
    try:
        text = cache.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return None
    match = re.search(r"^CMAKE_BUILD_TYPE:\w+=(.*)$", text, re.M)
...
def _measure(driver: Path, key_class: str, workdir: Path) -> Optional[dict[str, int]]:
    ...
    Disabling the auto-tune pins the subject to the library's default wiring
    — a fixed, named target rather than a host-dependent one — and the report
    below prints that wiring so the evidence says what it covers.  The Keccak
    paragraph above records the state this defect was found in; since the
    twenty-third maintenance pass the single-state AVX2 Keccak is no longer
    installed by the dispatcher at all (it retired about three times the
    instructions of the BMI2 scalar kernel and every measured host's
    auto-tune reverted it), so on x86-64 the default wiring the gate measures
    for ``keccak_f1600`` IS the scalar BMI2 kernel production runs.  Kyber and
    Dilithium NTTs still measure the default AVX2 wiring; ``AMA_DISPATCH_ONLY``
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        "instruction-count driver under callgrind: the secret is marked "
        "undefined and any conditional jump, conditional move or memory "
        "address that depends on it is reported (see _TAINT_DRIVERS).",
    )
    parser.add_argument(
        "--optimized-witness",
        type=Path,
        default=None,
        help="A CMakeCache.txt proving the build type of a library that does "
        "not export ama_build_optimization_probe() (the shipped shared object "
        "is built without AMA_TESTING_MODE).  Accepted only if it records an "
    ...
    if args.taint and args.target not in _TAINT_DRIVERS:
        print(
            ...
            f"{args.target!r}; taint targets: {', '.join(sorted(_TAINT_DRIVERS))}",
            file=sys.stderr,
        )
        return 2
    ...
    with tempfile.TemporaryDirectory(prefix="ama-ghash-ct-") as tmp:
        ...
        source.write_text(
            _TAINT_DRIVERS[args.target] if args.taint else _DRIVERS[args.target],
            encoding="utf-8",
        )
        ...
        witness_note = ""
        if optimized is None and args.optimized_witness is not None:
            # The shipped shared object carries no AMA_TESTING_MODE export, so
            # the probe cannot be linked; a CMakeCache.txt from the build that
            # produced it is the next-best witness of its optimization level,
            # and it is named in the report so the reader knows the verdict
            # rests on it rather than on the library's own word.
            build_type = _cmake_build_type(args.optimized_witness)
            if build_type in ("Release", "RelWithDebInfo", "MinSizeRel"):
                optimized = 1
                witness_note = (
                    f"optimization witness: {args.optimized_witness} "
                    f"(CMAKE_BUILD_TYPE={build_type}; the library exports no probe)"
                )
        if optimized != 1:
            reason = {
                0: (
        ...
        if args.taint:
            ...
            if taint is None:
                print(
                    "CONSTANT-TIME CHECK INCONCLUSIVE — the taint driver did not "
                    "run to completion under Memcheck.",
                    file=sys.stderr,
                )
                return 2
            reports, log = taint
            print(f"[{args.target}] secret-taint analysis under Memcheck:")
            for line in wiring:
                print(f"  wiring: {line}")
            if witness_note:
                print(f"  {witness_note}")
            if reports:
                print(
                    f"  {reports} report(s) of a conditional jump/move or memory address "
                ...
                for line in log.splitlines():
                    ...
                    shown += 1
                    if shown >= 80:
                        print("  ... (memcheck log truncated)")
                        break
                print(
                    f"\n{args.target.upper()} SECRET-TAINT CHECK FAILED — a control-flow "
                    "decision or a memory address depends on the secret."
                )
                return 1
            print(
                f"\n{args.target.upper()} SECRET-TAINT CHECK PASSED — no conditional jump, "
                "conditional move or memory address depends on the secret."
            )
            return 0
