...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for tool in ("valgrind", args.cc):
        if shutil.which(tool) is None:
            print(
                f"GHASH CONSTANT-TIME CHECK INCONCLUSIVE — {tool!r} is not installed.",
                file=sys.stderr,
            )
            return 2
    if not args.lib.is_file():
        print(
            f"GHASH CONSTANT-TIME CHECK INCONCLUSIVE — {args.lib} does not exist.\n"
    ...
    with tempfile.TemporaryDirectory(prefix="ama-ghash-ct-") as tmp:
        ...
        if compiled.returncode != 0:
            print(
                "GHASH CONSTANT-TIME CHECK INCONCLUSIVE — the driver did not build.\n"
                f"{' '.join(compile_cmd)}\n{compiled.stderr}",
                file=sys.stderr,
            )
            return 2
        ...
        if floor_a is None or floor_b is None:
            print(
                "GHASH CONSTANT-TIME CHECK INCONCLUSIVE — callgrind produced no "
                "instruction count.",
                file=sys.stderr,
            )
            return 2
        ...
        if floor > args.threshold:
            print(
                f"GHASH CONSTANT-TIME CHECK INCONCLUSIVE — the same key twice "
        ...
        for key_class in KEY_CLASSES[1:]:
            ...
            if count is None:
                print(
                    "GHASH CONSTANT-TIME CHECK INCONCLUSIVE — callgrind produced "
    ...
    print("Scalar AES-256-GCM retired-instruction counts by key class:")
    ...
    if worst > args.threshold:
        print(
            "\nGHASH CONSTANT-TIME CHECK FAILED — the scalar AES-GCM path retires a\n"
            "key-dependent number of instructions. The usual cause is an optimizer\n"
            "turning the masked GHASH accumulation back into a branch on the secret\n"
            "subkey; see src/c/internal/ama_ct_barrier.h. Disassemble ghash_mul in\n"
            "the built object and look for a conditional branch that skips the\n"
            "16-byte XOR.",
            file=sys.stderr,
        )
        return 1

    print("\nGHASH CONSTANT-TIME CHECK PASSED — instruction count is key-independent.")
    return 0
