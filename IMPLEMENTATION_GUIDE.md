...
| `AMA_ED25519_VERIFY_SHAMIR` | 1 | PR-B Ed25519 verify path. `1` = Shamir/Straus joint scalar mult `[s]B + [h](-A)` in one interleaved pass (default, ~30% fewer point doublings vs split layout). `0` = pre-PR-B sequential layout (`[s]B` via comb + `[h](-A)` via wNAF + final point add). Kept for one release as a one-flag rollback hatch; both paths are byte-identical at the `R_check` level (verified by `tests/c/test_ed25519_verify_equiv.c` layer B). |
| `AMA_ED25519_VERIFY_WINDOW` | 5 | wNAF window width used by the variable-base scalar mults inside the Ed25519 verify path (both Shamir joint mult and the legacy split mult). Accepted range: `[2, 6]`. Stack table size per point is `2^(W-1) * sizeof(ge25519_p3)` ≈ 160 B × `2^(W-1)` (Shamir instantiates two such tables). Practical sweet spot is `W=5` (≈18% fewer adds vs `W=4`); `W=6` is roughly par on Curve25519 with a doubled stack footprint. `W=4` is the documented rollback default to the pre-PR-B layout. |
